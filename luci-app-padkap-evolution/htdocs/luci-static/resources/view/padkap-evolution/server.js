...
const SERVER_STYLE_ID = "padkap-evolution-server-styles";
...
function defaultTlsCertificatePath(sectionId) {
  return `/etc/padkap-evolution/server-certs/${serverSafeName(sectionId)}.crt`;
}

function defaultTlsKeyPath(sectionId) {
  return `/etc/padkap-evolution/server-certs/${serverSafeName(sectionId)}.key`;
}

function defaultTailscaleHostname(sectionId) {
  return `padkap-evolution-${serverSafeName(sectionId)}`;
}
...
function loadGeneratedTlsCertificatePin(sectionId) {
  ...
  return fs
    .exec("/usr/bin/padkap-evolution", ["get_tls_certificate_sha256", sectionId])
    .then((response) => {
...
function generateRealityKeypair(sectionId) {
  return fs
    .exec("/usr/bin/padkap-evolution", ["generate_reality_keypair"])
    .then((response) => {
      if ((response.code ?? 0) !== 0 || !response.stdout) {
        throw new Error(response.stderr || response.stdout || "");
      }
