...
// Global settings
"require view.padkap-evolution.settings as settings";

// Sections
"require view.padkap-evolution.section as section";

// Server
"require view.padkap-evolution.server as server";

// Dashboard
"require view.padkap-evolution.dashboard as dashboard";

// Monitoring
"require view.padkap-evolution.monitoring as monitoring";

// Diagnostic
"require view.padkap-evolution.diagnostic as diagnostic";

// Updates
"require view.padkap-evolution.updates as updates";
  ...
  async render() {
    ...
    const applyUiCapabilities = function () {
      ...
      if (typeof window !== "undefined") {
        window.dispatchEvent(
          new CustomEvent(main.PADKAP_EVOLUTION_ACTION_PROVIDERS_AVAILABILITY_EVENT, {
            detail: {
        ...
        main.store.set({
          servicesInfoWidget: {
            ...
            data: {
              ...
              padkapEvolutionRunning: Number(data.service["padkap-evolution"]?.running) || 0,
              padkapEvolutionEnabled: Number(data.service["padkap-evolution"]?.enabled) || 0,
              padkapEvolutionStatus: data.service["padkap-evolution"]?.status || "",
            },
          },
        });
      }
    ...
    const loadFallbackUiCapabilities = function () {
      return Promise.allSettled([
        main.PadkapEvolutionShellMethods.getServerCapabilities(),
        ...
        main.PadkapEvolutionShellMethods.checkInboundsConfig(),
      ]).then(
        ([
    ...
    const loadUiCapabilities = function () {
      ...
      uiCapabilitiesPromise = main.PadkapEvolutionShellMethods.getUiCapabilities()
        .then((response) => {
          if (!response?.success) {
        ...
        .catch((error) => {
          console.warn("Failed to load Padkap Evolution UI capabilities", error);
          return main.PadkapEvolutionShellMethods.getUiState()
            .then((response) => {
              if (!response?.success) {
            ...
            .catch((fallbackError) => {
              console.warn("Failed to load Padkap Evolution UI state", fallbackError);
    ...
    const padkapEvolutionMap = new form.Map(
      UCI_PACKAGE,
      _("Padkap Evolution Settings"),
      _("Configuration for Padkap Evolution service"),
    );
    padkapEvolutionMap.tabbed = true;
    const originalHandleSaveApply = padkapEvolutionMap.handleSaveApply;
    padkapEvolutionMap.handleSaveApply = function (ev, mode) {
      const refreshUiState = function () {
        main.PadkapEvolutionShellMethods.getUiState()
          .then((response) => {
            if (
              response?.success &&
        ...
        main.store.set({
          servicesInfoWidget: {
            ...
            data: {
              ...
              padkapEvolutionStatus: "reloading",
            },
          },
        });
      }
    ...
    const rulesSection = padkapEvolutionMap.section(
      form.GridSection,
      "section",
      _("Sections"),
    ...
    const serverSection = padkapEvolutionMap.section(
      form.GridSection,
      "server",
      _("Servers"),
    ...
    const settingsSection = padkapEvolutionMap.section(
      form.TypedSection,
      "settings",
      _("Settings"),
    );
    ...
    const diagnosticSection = padkapEvolutionMap.section(
      form.TypedSection,
      "diagnostic",
      _("Diagnostics"),
    );
    ...
    const dashboardSection = padkapEvolutionMap.section(
      form.TypedSection,
      "dashboard",
      _("Dashboard"),
    );
    ...
    const monitoringSection = padkapEvolutionMap.section(
      form.TypedSection,
      "monitoring",
      _("Monitoring"),
    );
    ...
    const updatesSection = padkapEvolutionMap.section(
      form.TypedSection,
      "updates",
      _("Components"),
    );
    ...
    const rendered = await padkapEvolutionMap.render();
    main.coreService({
