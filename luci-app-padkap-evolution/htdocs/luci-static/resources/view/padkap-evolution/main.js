...
// src/padkap-evolution/tabs/dashboard/partials/renderFlagEmojis.ts
...
// src/padkap-evolution/services/logger.service.ts
var Logger = class {
  constructor() {
    this.logs = [];
...
// src/constants.ts
var PADKAP_EVOLUTION_UCI_PACKAGE = "padkap-evolution";
var PADKAP_EVOLUTION_LUCI_APP_VERSION = "__COMPILED_VERSION_VARIABLE__";
var PADKAP_EVOLUTION_ACTION_PROVIDERS_AVAILABILITY_EVENT = "padkap-evolution:action-providers-availability";
...
// src/padkap-evolution/tabs/dashboard/partials/renderSections.ts
...
// src/padkap-evolution/tabs/dashboard/partials/renderWidget.ts
...
// src/padkap-evolution/tabs/dashboard/render.ts
function render() {
  return E(
    "div",
    {
...
// src/padkap-evolution/methods/custom/getConfigSections.ts
async function getConfigSections() {
  return uci.load(PADKAP_EVOLUTION_UCI_PACKAGE).then(() => uci.sections(PADKAP_EVOLUTION_UCI_PACKAGE));
}

// src/padkap-evolution/runtimeTags.ts
...
// src/padkap-evolution/methods/shell/callBaseMethod.ts
async function callBaseMethod(method, args = [], command = "/usr/bin/padkap-evolution", options = {}) {
  try {
...
((PadkapEvolution2) => {
  ...
  })(AvailableMethods = PadkapEvolution2.AvailableMethods || (PadkapEvolution2.AvailableMethods = {}));
  ...
  })(AvailableClashAPIMethods = PadkapEvolution2.AvailableClashAPIMethods || (PadkapEvolution2.AvailableClashAPIMethods = {}));
})(PadkapEvolution || (PadkapEvolution = {}));

// src/padkap-evolution/helpers/isTransientRpcError.ts
...
// src/padkap-evolution/methods/shell/index.ts
...
var COMPONENT_ACTION_STATE_DIR = "/var/run/padkap-evolution/component-actions";
...
var PadkapEvolutionShellMethods = {
  ...
  getStatus: async () => callBaseMethod(
  ...
  getSubscriptionMetadata: async (section) => callBaseMethod(PadkapEvolution.AvailableMethods.GET_SUBSCRIPTION_METADATA, [section]),
  ...
  getClashApiProxies: async () => callBaseMethod(
    PadkapEvolution.AvailableMethods.CLASH_API,
    [PadkapEvolution.AvailableClashAPIMethods.GET_PROXIES]
  ),
  getClashApiConnections: async () => callBaseMethod(PadkapEvolution.AvailableMethods.CLASH_API, [
  ...
  setClashApiGroupProxy: async (group, proxy) => callBaseMethod(PadkapEvolution.AvailableMethods.CLASH_API, [
    PadkapEvolution.AvailableClashAPIMethods.SET_GROUP_PROXY,
    group,
    proxy
  ]),
  closeClashApiConnection: async (connectionId) => callBaseMethod(PadkapEvolution.AvailableMethods.CLASH_API, [
    PadkapEvolution.AvailableClashAPIMethods.CLOSE_CONNECTION,
    connectionId
  ]),
  closeAllClashApiConnections: async () => callBaseMethod(PadkapEvolution.AvailableMethods.CLASH_API, [
  ...
  globalCheck: async (masked = true) => callBaseMethod(PadkapEvolution.AvailableMethods.GLOBAL_CHECK, [
    masked ? "masked" : "raw"
  ]),
  showSingBoxConfig: async (masked = true) => callBaseMethod(
    PadkapEvolution.AvailableMethods.SHOW_SING_BOX_CONFIG,
    [masked ? "masked" : "raw"]
  ),
  checkLogs: async () => callBaseMethod(PadkapEvolution.AvailableMethods.CHECK_LOGS),
  checkSingBoxLogs: async () => callBaseMethod(
  ...
  waitServiceActionJob: async (jobId, startedAt = Date.now()) => {
    while (Date.now() - startedAt < SERVICE_ACTION_TIMEOUT_MS) {
      ...
      const response = await PadkapEvolutionShellMethods.serviceActionStatus(jobId);
      if (!response.success) {
        return response;
      }
  ...
  waitLatencyTestJob: async (jobId, startedAt = Date.now()) => {
    ...
    while (Date.now() - startedAt < LATENCY_TEST_TIMEOUT_MS) {
      ...
      const response = await PadkapEvolutionShellMethods.latencyTestStatus(jobId);
      if (!response.success) {
    ...
    while (true) {
      ...
      if ((statusResponse.code ?? 0) !== 0 || !parsedResponse) {
        ...
        if (component === "padkap-evolution" && action === "install") {
          const installedVersion = expectedLatestVersion ? await readPadkapEvolutionVersion() : "";
  ...
  waitSubscriptionUpdateJob: async (jobId) => {
    ...
    while (true) {
      ...
      const response = await PadkapEvolutionShellMethods.subscriptionUpdateStatus(jobId);
      if (!response.success) {
        if (transientRpc.shouldContinue(response.error)) {
          continue;
        }
...
// src/padkap-evolution/methods/custom/getDashboardSections.ts
var DASHBOARD_SECTION_CACHE_DIR = "/var/run/padkap-evolution/section-cache";
...
// src/padkap-evolution/methods/custom/getClashApiSecret.ts
...
// src/padkap-evolution/methods/custom/index.ts
var CustomPadkapEvolutionMethods = {
  getConfigSections,
  getDashboardSections,
  getClashApiSecret: getClashApiSecret2
};

// src/padkap-evolution/api.ts
...
// src/padkap-evolution/methods/fakeip/getFakeIpCheck.ts
...
// src/padkap-evolution/methods/fakeip/getIpCheck.ts
...
// src/padkap-evolution/methods/fakeip/index.ts
var RemoteFakeIPMethods = {
  getFakeIpCheck,
  getIpCheck
};

// src/padkap-evolution/services/tab.service.ts
...
// src/padkap-evolution/tabs/diagnostic/helpers/getCheckTitle.ts
function getCheckTitle(name) {
  return `${name} ${_("checks")}`;
}

// src/padkap-evolution/tabs/diagnostic/checks/contstants.ts
...
// src/padkap-evolution/tabs/diagnostic/diagnostic.store.ts
...
// src/padkap-evolution/services/store.service.ts
...
// src/padkap-evolution/services/padkapEvolutionLogWatcher.service.ts
var PadkapEvolutionLogWatcher = class _PadkapEvolutionLogWatcher {
  constructor() {
    this.intervalMs = 5e3;
  ...
  static getInstance() {
    if (!_PadkapEvolutionLogWatcher.instance) {
      _PadkapEvolutionLogWatcher.instance = new _PadkapEvolutionLogWatcher();
    }
    return _PadkapEvolutionLogWatcher.instance;
  }
...
// src/padkap-evolution/services/logNotificationDeduper.service.ts
var LOG_NOTIFICATION_STORAGE_KEY = "padkap-evolution:shown-log-error-notifications:v1";
...
function getPadkapEvolutionLogNotification(line) {
...
// src/padkap-evolution/helpers/getComponentActionKey.ts
...
// src/padkap-evolution/helpers/singBoxVariant.ts
...
// src/padkap-evolution/services/localActionOverlay.service.ts
...
// src/padkap-evolution/services/uiState.service.ts
...
// src/padkap-evolution/services/runtimeUiState.service.ts
...
async function refreshRuntimeUiState({
  ...
  const promise = PadkapEvolutionShellMethods.getUiState().then((response) => {
    if (!response.success) {
      return void 0;
    }
...
// src/padkap-evolution/services/core.service.ts
...
// src/padkap-evolution/services/socket.service.ts
...
// src/padkap-evolution/services/uiActionNotification.service.ts
var UI_ACTION_NOTIFICATION_STORAGE_KEY = "padkap-evolution:owned-ui-action-notifications:v1";
...
// src/padkap-evolution/fetchers/fetchServicesInfo.ts
  ...
  store.set({
    servicesInfoWidget: {
      ...
      data: {
        ...
        padkapEvolutionRunning: padkapEvolution.success ? padkapEvolution.data.running : previousData.padkapEvolutionRunning,
        padkapEvolutionEnabled: padkapEvolution.success ? padkapEvolution.data.enabled : previousData.padkapEvolutionEnabled,
        padkapEvolutionStatus: padkapEvolution.success ? padkapEvolution.data.status : previousData.padkapEvolutionStatus
      }
    }
  });
  return void 0;
...
// src/padkap-evolution/helpers/isActiveLuciTab.ts
...
// src/padkap-evolution/helpers/restoredActionLoading.ts
function shouldShowLoadingForRestoredAction(state) {
  return state.running === true;
}

// src/padkap-evolution/helpers/serviceAvailability.ts
...
// src/padkap-evolution/tabs/dashboard/initController.ts
...
async function fetchDashboardSectionsOnce(mountId) {
  ...
  try {
    const { data, success } = await CustomPadkapEvolutionMethods.getDashboardSections();
...
async function followSubscriptionUpdateState(state) {
  ...
  try {
    const response = state.running ? await PadkapEvolutionShellMethods.waitSubscriptionUpdateJob(jobId) : {
      success: true,
      data: state
    };
...
async function handleUpdateSubscription(section) {
  ...
  let ownsJobFollow = false;
  try {
    const startResponse = await PadkapEvolutionShellMethods.subscriptionUpdateStart(
      section.sectionName
    );
    if (!startResponse.success) {
      throw new Error(startResponse.error);
    ...
    followedSubscriptionJobs.add(jobId);
    ownsJobFollow = true;
    const response = await PadkapEvolutionShellMethods.waitSubscriptionUpdateJob(jobId);
    ...
    items: [
      {
        ...
        value: servicesInfoWidget.data.padkapEvolutionRunning ? _("\u2714 Running") : _("\u2718 Stopped"),
        attributes: {
          class: servicesInfoWidget.data.padkapEvolutionRunning ? "fkp_dashboard-page__widgets-section__item__row--success" : "fkp_dashboard-page__widgets-section__item__row--error"
        }
      },
      {
        key: "Sing-box",
...
// src/padkap-evolution/tabs/dashboard/styles.ts
var styles = `
@font-face {
...
#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-dashboard-_mount_node > .cbi-value-title {
    display: none;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-dashboard-_mount_node > .cbi-value-field {
    margin-left: 0;
    width: 100%;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-dashboard-_mount_node > div {
    width: 100%;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-dashboard > h3 {
    display: none;
}
...
// src/padkap-evolution/tabs/dashboard/index.ts
var DashboardTab = {
  render,
  initController,
  styles
...
// src/padkap-evolution/tabs/diagnostic/renderDiagnostic.ts
function render2() {
...
// src/padkap-evolution/tabs/diagnostic/checks/updateCheckStore.ts
...
// src/padkap-evolution/tabs/diagnostic/helpers/getMeta.ts
...
// src/padkap-evolution/tabs/diagnostic/checks/getDnsCheckPresentation.ts
...
// src/padkap-evolution/tabs/diagnostic/checks/runDnsCheck.ts
...
// src/padkap-evolution/tabs/diagnostic/checks/runSingBoxCheck.ts
...
// src/padkap-evolution/tabs/diagnostic/checks/runInboundsCheck.ts
...
// src/padkap-evolution/tabs/diagnostic/checks/runNftCheck.ts
...
// src/padkap-evolution/tabs/diagnostic/checks/runFakeIPCheck.ts
...
// src/padkap-evolution/tabs/diagnostic/checks/getCheckItemsMeta.ts
...
// src/padkap-evolution/tabs/diagnostic/checks/runZapretCheck.ts
async function runZapretCheck() {
  ...
  const padkapEvolutionRuntimeReady = !hasZapretRules || runningProcesses === expectedProcesses && supervisorProcesses === expectedProcesses;
  ...
  const items = [
    ...
    {
      state: unexpectedRuntime || !padkapEvolutionRuntimeReady ? "error" : "success",
      key: hasZapretRules ? padkapEvolutionRuntimeReady ? _("Padkap Evolution-managed nfqws runtime is ready") : _("Padkap Evolution-managed nfqws runtime is not ready") : unexpectedRuntime ? _("Unexpected Padkap Evolution-managed nfqws runtime is running") : _("Padkap Evolution-managed nfqws runtime is not running"),
...
// src/padkap-evolution/tabs/diagnostic/checks/runZapret2Check.ts
async function runZapret2Check() {
  ...
  const runningProcesses = Number(data.running_process_count || 0);
  const supervisorProcesses = Number(data.supervisor_process_count || 0);
  const padkapEvolutionRuntimeReady = !hasZapret2Rules || runningProcesses === expectedProcesses && supervisorProcesses === expectedProcesses;
  ...
  const items = [
    ...
    {
      state: unexpectedRuntime || !padkapEvolutionRuntimeReady ? "error" : "success",
      key: hasZapret2Rules ? padkapEvolutionRuntimeReady ? _("Padkap Evolution-managed nfqws2 runtime is ready") : _("Padkap Evolution-managed nfqws2 runtime is not ready") : unexpectedRuntime ? _("Unexpected Padkap Evolution-managed nfqws2 runtime is running") : _("Padkap Evolution-managed nfqws2 runtime is not running"),
...
// src/padkap-evolution/tabs/diagnostic/checks/runByedpiCheck.ts
async function runByedpiCheck() {
  ...
  const padkapEvolutionRuntimeReady = !hasByedpiRules || runningProcesses === expectedProcesses && supervisorProcesses === expectedProcesses;
  ...
  const items = [
    ...
    {
      state: unexpectedRuntime || !padkapEvolutionRuntimeReady ? "error" : runtimeUnstable ? "warning" : "success",
      key: hasByedpiRules ? runtimeUnstable ? _("Padkap Evolution-managed ciadpi runtime has restarted") : padkapEvolutionRuntimeReady ? _("Padkap Evolution-managed ciadpi runtime is ready") : _("Padkap Evolution-managed ciadpi runtime is not ready") : unexpectedRuntime ? _("Unexpected Padkap Evolution-managed ciadpi runtime is running") : _("Padkap Evolution-managed ciadpi runtime is not running"),
...
// src/padkap-evolution/services/systemInfo.service.ts
...
// src/padkap-evolution/tabs/diagnostic/partials/renderAvailableActions.ts
...
// src/padkap-evolution/tabs/diagnostic/partials/renderCheckSection.ts
...
// src/padkap-evolution/tabs/diagnostic/partials/renderRunAction.ts
...
// src/padkap-evolution/tabs/diagnostic/partials/renderSystemInfo.ts
...
// src/padkap-evolution/tabs/diagnostic/partials/renderWikiDisclaimer.ts
...
// src/padkap-evolution/tabs/diagnostic/checks/runSectionsCheck.ts
    ...
    async function getLatency() {
      if (section.withTagSelect) {
        ...
        if (selectedOutbound2?.code) {
          const latencyProxy2 = await PadkapEvolutionShellMethods.getClashApiProxyLatency(
        ...
        const latencyGroup = await PadkapEvolutionShellMethods.getClashApiGroupLatency(
          section.code
        );
      ...
      const latencyProxy = await PadkapEvolutionShellMethods.getClashApiProxyLatency(
        section.code,
        section.latencyTestTimeout
      );
...
// src/padkap-evolution/tabs/diagnostic/serviceTransition.ts
...
function shouldDisableDiagnosticRunAction({
  ...
  return !providerInfoLoaded || servicesInfoLoading || !padkapEvolutionRunning || mutatingServiceActionLoading;
}
...
function shouldShowRestartAction({
  ...
  return restartLoading || padkapEvolutionRunning && !startLoading && !stopLoading;
}
function shouldShowStartAction({
  ...
  return startLoading || !restartLoading && !padkapEvolutionRunning && !stopLoading;
}
function shouldShowStopAction({
  ...
  return stopLoading || restartLoading || padkapEvolutionRunning && !startLoading;
}

// src/padkap-evolution/tabs/diagnostic/diagnosticRunPersistence.ts
var DIAGNOSTIC_RUN_STORAGE_KEY = "padkap-evolution:diagnostic-run:v1";
...
// src/padkap-evolution/tabs/diagnostic/helpers/maskDiagnostics.ts
...
var PADKAP_EVOLUTION_MASK_AFTER_TOKEN = [
  "option proxy_string",
  "option hwid",
...
var PADKAP_EVOLUTION_MASK_AFTER_TOKEN_SPACE = [
...
// src/padkap-evolution/tabs/diagnostic/initController.ts
...
function isMutatingServiceActionLoading() {
  return isLocalMutatingServiceActionLoading() || isServiceTransitionStatus(
    store.get().servicesInfoWidget.data.padkapEvolutionStatus
  );
}
function getPadkapEvolutionStatusText(running, enabled) {
  if (running) {
    return enabled ? "running & enabled" : "running but disabled";
  }
  return enabled ? "stopped but enabled" : "stopped & disabled";
}
function setDisplayedPadkapEvolutionRunning(running) {
  const servicesInfoWidget = store.get().servicesInfoWidget;
...
async function waitForPadkapEvolutionRunningState(expectedRunning) {
...
async function handleServiceRuntimeAction({
  ...
  try {
    const startResponse = await PadkapEvolutionShellMethods.serviceActionStart(action);
    if (!startResponse.success) {
      throw new Error(startResponse.error);
    }
    jobId = startResponse.data.job_id;
    ...
    followedServiceActionJobs.add(jobId);
    ownsJobFollow = true;
    const result = await PadkapEvolutionShellMethods.waitServiceActionJob(jobId);
    if (!result.success) {
...
async function handleShowSingBoxConfig() {
  ...
  try {
    const showSingBoxConfig = await PadkapEvolutionShellMethods.showSingBoxConfig(false);
...
// src/padkap-evolution/tabs/diagnostic/styles.ts
var styles4 = `

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-diagnostic-_mount_node > div {
    width: 100%;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-diagnostic > h3 {
    display: none;
}
...
// src/padkap-evolution/tabs/diagnostic/index.ts
var DiagnosticTab = {
  render: render2,
...
// src/padkap-evolution/tabs/monitoring/render.ts
function render3() {
  return E(
    "div",
    {
...
// src/padkap-evolution/tabs/monitoring/initController.ts
...
async function closeConnection(connectionId) {
  ...
  renderConnections();
  try {
    const response = await PadkapEvolutionShellMethods.closeClashApiConnection(connectionId);
    if (!response.success) {
...
async function closeAllConnections() {
  ...
  renderControls();
  try {
    const response = await PadkapEvolutionShellMethods.closeAllClashApiConnections();
    if (!response.success) {
...
// src/padkap-evolution/tabs/monitoring/styles.ts
var styles5 = `
#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-monitoring-_mount_node {
    margin: 16px 0 22px;
    padding: 0;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-monitoring-_mount_node > .cbi-value-title {
    display: none;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-monitoring-_mount_node > .cbi-value-field {
    margin-left: 0;
    width: 100%;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-monitoring-_mount_node > div {
    width: 100%;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-monitoring > h3 {
    display: none;
}
...
// src/padkap-evolution/tabs/monitoring/index.ts
var MonitoringTab = {
  render: render3,
...
// src/padkap-evolution/tabs/updates/render.ts
function render4() {
...
// src/padkap-evolution/tabs/updates/componentActionCompletion.ts
function shouldApplyCompletedComponentActionResult(result, notify) {
  return result.action !== "check_update" || notify;
}

// src/padkap-evolution/tabs/updates/checkResultLifecycle.ts
...
// src/padkap-evolution/tabs/updates/initController.ts
...
function isServiceRuntimeActionLoading() {
  ...
  return hasLocalMutatingServiceActionLoading(state.diagnosticsActions) || isServiceTransitionStatus(
...
function loadComponentUpdateCheckCache({ force = false } = {}) {
  ...
  const promise = PadkapEvolutionShellMethods.componentUpdateCheckCache().then(
...
async function followComponentActionState(state) {
  ...
  try {
    const response = state.running ? await PadkapEvolutionShellMethods.waitComponentActionJob(
      jobId,
      state.component,
      state.action,
...
async function handleComponentAction(button) {
  ...
  let ownsJobFollow = false;
  try {
    const startResponse = await PadkapEvolutionShellMethods.componentActionStart(
      button.component,
      button.action
    );
    if (!startResponse.success) {
  ...
  return [
    {
      ...
      version: systemInfoLoading ? _("Loading...") : normalizeCompiledVersion(systemInfo.padkap_evolution_version),
...
// src/padkap-evolution/tabs/updates/styles.ts
var styles6 = `
#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-updates-_mount_node > div {
    width: 100%;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-updates > h3 {
    display: none;
}
...
// src/padkap-evolution/tabs/updates/index.ts
var UpdatesTab = {
  render: render4,
...
#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-settings > h3 {
    display: none;
}

/* Hide extra H3 for rules tab */
#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-section > h3:nth-child(1) {
    display: none;
}

/* Vertical align for remove rule action button */
#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-section > .cbi-section-remove {
    margin-bottom: -32px;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-section .cbi-section-actions > div {
    display: inline-flex;
    align-items: center;
    gap: 4px;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-section .cbi-section-actions {
    text-align: right;
}

/* Rule reorder visuals */
#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-section {
    position: relative;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-section .cbi-section-table-row {
    position: relative;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-section .cbi-section-table-row.placeholder {
    opacity: 1;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-section .cbi-section-table-row.placeholder em {
    font-style: italic;
}

#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-section .cbi-section-table-row.drag-over-above::after,
#cbi-${PADKAP_EVOLUTION_UCI_PACKAGE}-section .cbi-section-table-row.drag-over-below::after {
    content: '';
    position: absolute;
    left: 10px;
    right: 10px;
...
var PADKAP_EVOLUTION_GLOBAL_STYLES_ID = "padkap-evolution-global-styles";
