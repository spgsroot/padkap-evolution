...
const UCI_PACKAGE = main.PADKAP_EVOLUTION_UCI_PACKAGE;
const ACTION_PROVIDERS_AVAILABILITY_EVENT =
  main.PADKAP_EVOLUTION_ACTION_PROVIDERS_AVAILABILITY_EVENT ||
  "padkap-evolution:action-providers-availability";
...
const NFQWS_VALIDATION_COMMAND = "/usr/bin/padkap-evolution";
...
const SECTION_CACHE_DIR = "/var/run/padkap-evolution/section-cache";
const COUNTRY_CODES =
...
function ensureActionProvidersAvailabilityLoaded() {
  ...
  actionProvidersAvailabilityPromise = Promise.allSettled([
    main.PadkapEvolutionShellMethods.checkZapretRuntime(),
    main.PadkapEvolutionShellMethods.checkZapret2Runtime(),
    main.PadkapEvolutionShellMethods.checkByedpiRuntime(),
  ])
...
function attachAnnotatedTextarea(textarea, analyzer) {
  ...
  if (textarea.__padkapEvolutionAnnotatedTextareaController) {
    textarea.__padkapEvolutionAnnotatedTextareaController.analyzer = analyzer;
    textarea.__padkapEvolutionAnnotatedTextareaController.update();
    return;
  }
  ...
  textarea.__padkapEvolutionAnnotatedTextareaController = controller;
...
function refreshAnnotatedTextareaValidation(option, section_id, textarea) {
  ...
  if (
    textarea &&
    textarea.__padkapEvolutionAnnotatedTextareaController &&
    typeof textarea.__padkapEvolutionAnnotatedTextareaController.update === "function"
  ) {
    textarea.__padkapEvolutionAnnotatedTextareaController.update();
  }
}
...
function attachNfqwsRemoteValidation(option, section_id, textarea) {
  if (!textarea || textarea.__padkapEvolutionNfqwsRemoteValidationAttached) {
    return;
  }

  textarea.__padkapEvolutionNfqwsRemoteValidationAttached = true;
  textarea.__padkapEvolutionNfqwsRemoteValidationRequestId = 0;
  textarea.__padkapEvolutionNfqwsRemoteValidationTimer = null;
  ...
  const runValidation = () => {
    ...
    const requestId =
      (textarea.__padkapEvolutionNfqwsRemoteValidationRequestId || 0) + 1;
    textarea.__padkapEvolutionNfqwsRemoteValidationRequestId = requestId;

    validateNfqwsStrategyRemotely(value).then(() => {
      if (textarea.__padkapEvolutionNfqwsRemoteValidationRequestId !== requestId) {
        return;
      }
  ...
  const scheduleValidation = (delay = NFQWS_REMOTE_VALIDATION_DEBOUNCE_MS) => {
    if (textarea.__padkapEvolutionNfqwsRemoteValidationTimer) {
      window.clearTimeout(textarea.__padkapEvolutionNfqwsRemoteValidationTimer);
    }

    textarea.__padkapEvolutionNfqwsRemoteValidationTimer = window.setTimeout(() => {
      textarea.__padkapEvolutionNfqwsRemoteValidationTimer = null;
      runValidation();
    }, delay);
  };
...
function attachNfqws2RemoteValidation(option, section_id, textarea) {
  if (!textarea || textarea.__padkapEvolutionNfqws2RemoteValidationAttached) {
    return;
  }

  textarea.__padkapEvolutionNfqws2RemoteValidationAttached = true;
  textarea.__padkapEvolutionNfqws2RemoteValidationRequestId = 0;
  textarea.__padkapEvolutionNfqws2RemoteValidationTimer = null;
  ...
  const runValidation = () => {
    ...
    const requestId =
      (textarea.__padkapEvolutionNfqws2RemoteValidationRequestId || 0) + 1;
    textarea.__padkapEvolutionNfqws2RemoteValidationRequestId = requestId;

    validateNfqws2StrategyRemotely(value).then(() => {
      if (textarea.__padkapEvolutionNfqws2RemoteValidationRequestId !== requestId) {
        return;
      }
  ...
  const scheduleValidation = (delay = NFQWS_REMOTE_VALIDATION_DEBOUNCE_MS) => {
    if (textarea.__padkapEvolutionNfqws2RemoteValidationTimer) {
      window.clearTimeout(textarea.__padkapEvolutionNfqws2RemoteValidationTimer);
    }

    textarea.__padkapEvolutionNfqws2RemoteValidationTimer = window.setTimeout(() => {
      textarea.__padkapEvolutionNfqws2RemoteValidationTimer = null;
      runValidation();
    }, delay);
  };
...
function getNfqwsForbiddenTokenInfo(token, index) {
  const configFileMessage = _(
    "External nfqws config files bypass Padkap Evolution queue management and explicit validation.",
  );
  ...
  const placeholderMessage = _(
    "Zapret hostlist templates are not supported here because Padkap Evolution does not expand them for per-rule NFQWS strategies.",
  );
  const queueMessage = _(
    "The NFQUEUE number is assigned by Padkap Evolution for each rule and must not be overridden here.",
  );
  const fwmarkMessage = _(
    "The desync fwmark is managed by Padkap Evolution for loop prevention and must not be overridden here.",
  );
  const daemonMessage = _(
    "Padkap Evolution manages the nfqws process lifecycle itself, so daemon mode is not allowed here.",
  );
...
function getNfqws2ForbiddenTokenInfo(token, index, nextToken) {
  const configFileMessage = _(
    "External nfqws2 config files bypass Padkap Evolution queue management and explicit validation.",
  );
  const hostSelectionMessage = _(
  ...
  const placeholderMessage = _(
    "Zapret2 hostlist templates are not supported here because Padkap Evolution does not expand them for per-rule NFQWS2 strategies.",
  );
  const queueMessage = _(
    "The NFQUEUE number is assigned by Padkap Evolution for each rule and must not be overridden here.",
  );
  ...
  const fuzzMessage = _(
    "Fuzzing is not supported here because Padkap Evolution needs deterministic runtime validation.",
  );
  const interceptMessage = _(
    "Disabling interception is incompatible with action=zapret2 because Padkap Evolution sends matched traffic through NFQUEUE.",
  );
  const daemonMessage = _(
    "Padkap Evolution manages the nfqws2 process lifecycle itself, so daemon mode is not allowed here.",
  );
  const dryRunMessage = _(
...
function getByedpiControlledTokenInfo(token) {
  const listenMessage = _(
    "ByeDPI listen address and port are assigned by Padkap Evolution and must not be set in the strategy.",
  );
  const transparentMessage = _(
    "Transparent proxy mode is incompatible with action=byedpi because Padkap Evolution connects to ciadpi through SOCKS.",
  );
  const daemonMessage = _(
    "Padkap Evolution manages the ciadpi process lifecycle itself, so daemon mode is not allowed here.",
  );
  const pidfileMessage = _(
    "Padkap Evolution manages ciadpi pid files itself, so pidfile options are not allowed here.",
  );
  const exitMessage = _(
...
function createSectionContent(section) {
  ...
  o = section.taboption(
    ...
    "action",
    _("Action"),
    _("What Padkap Evolution should do when this section matches"),
  );
