...
const DEFAULT_PENDING_RELOAD_FILE = getenv("PADKAP_EVOLUTION_PENDING_RELOAD_FILE") || "/var/run/padkap-evolution/reload.pending";
const DEFAULT_SERVICE_INIT = getenv("PADKAP_EVOLUTION_SERVICE_INIT") || "/etc/init.d/padkap-evolution";
...
function run_pending_reload_if_requested(path, init_script) {
    ...
    command_success_from_args([ "logger", "-t", "padkap-evolution", "[info] Applying pending Padkap Evolution reload" ]);
...
function cleanup_rule_condition_cache(path) {
    ...
    system("rm -rf /tmp/padkap-evolution-rule-cache.* >/dev/null 2>&1");
}
...
function hup_sing_box_runtime() {
    ...
    command_success_from_args([ "logger", "-t", "padkap-evolution", "[info] Applying DNS failover with sing-box SIGHUP reload" ]);
...
function reload_sing_box_runtime(previous_pid, config_hash_before, config_hash_after, force) {
    if (!sing_box_runtime_reload_needed(config_hash_before, config_hash_after, force)) {
        command_success_from_args([ "logger", "-t", "padkap-evolution", "[info] sing-box reload skipped: configuration is unchanged" ]);
        return;
    }

    previous_pid = sing_box_reload_previous_pid(previous_pid, config_hash_before, config_hash_after);
    command_success_from_args([ "logger", "-t", "padkap-evolution", "[info] Reloading sing-box runtime" ]);
    if (!command_success_from_args([ "/etc/init.d/sing-box", "reload" ])) {
        command_success_from_args([ "logger", "-t", "padkap-evolution", "[fatal] Failed to reload sing-box. Aborted." ]);
        exit(1);
    }

    let timeout = getenv("PADKAP_EVOLUTION_SING_BOX_RELOAD_PID_TIMEOUT") || "15";
    if (previous_pid > 0 && !wait_sing_box_pid_replacement(previous_pid, timeout)) {
        command_success_from_args([ "logger", "-t", "padkap-evolution", "[fatal] sing-box reload did not replace the running process. Aborted." ]);
        exit(1);
    }
}
...
function padkap_evolution_runtime_network_configured(rt_table, nft_table, mark) {
...
function padkap_evolution_running(rt_table, nft_table, mark) {
    return sing_box_service_running() && sing_box_runtime_ports_ready() &&
        padkap_evolution_runtime_network_configured(rt_table, nft_table, mark);
}

function padkap_evolution_stably_running(rt_table, nft_table, mark, min_age) {
    return sing_box_service_stable(min_age) && sing_box_runtime_ports_ready() &&
        padkap_evolution_runtime_network_configured(rt_table, nft_table, mark);
}

function wait_padkap_evolution_stable_start(rt_table, nft_table, mark, min_age, timeout) {
    timeout = int(timeout || 8);
    while (timeout > 0) {
        if (padkap_evolution_stably_running(rt_table, nft_table, mark, min_age))
            return true;
    ...
    return padkap_evolution_stably_running(rt_table, nft_table, mark, min_age);
}
...
function dnsmasq_signature_body(settings, dnsmasq, legacy_dnsmasq_present) {
    ...
    body = signature_add_value(body, "dhcp.@dnsmasq[0].padkap_evolution_server", option(dnsmasq, "padkap_evolution_server", ""));
    body = signature_add_value(body, "dhcp.@dnsmasq[0].padkap_evolution_noresolv", option(dnsmasq, "padkap_evolution_noresolv", ""));
    body = signature_add_value(body, "dhcp.@dnsmasq[0].padkap_evolution_cachesize", option(dnsmasq, "padkap_evolution_cachesize", ""));
    body = signature_add_value(body, "dhcp.padkap-evolution.present", arg_bool(legacy_dnsmasq_present) ? "1" : "0");
...
function uci_dnsmasq() {
    return {
        ...
        padkap_evolution_server: uci_get("dhcp.@dnsmasq[0].padkap_evolution_server"),
        padkap_evolution_noresolv: uci_get("dhcp.@dnsmasq[0].padkap_evolution_noresolv"),
        padkap_evolution_cachesize: uci_get("dhcp.@dnsmasq[0].padkap_evolution_cachesize")
    };
}
...
function current_reload_state_values(format) {
    ...
    return reload_state_values_from_sources(
        ...
        uci_servers(),
        uci_dnsmasq(),
        uci_exists("dhcp.padkap-evolution"),
        mwan3_active()
    );
}
...
if (mode == "file-first-line")
...
else if (mode == "padkap-evolution-running")
    exit(padkap_evolution_running(ARGV[1], ARGV[2], ARGV[3]) ? 0 : 1);
else if (mode == "padkap-evolution-stably-running")
    exit(padkap_evolution_stably_running(ARGV[1], ARGV[2], ARGV[3], ARGV[4]) ? 0 : 1);
else if (mode == "wait-padkap-evolution-stable-start")
    exit(wait_padkap_evolution_stable_start(ARGV[1], ARGV[2], ARGV[3], ARGV[4], ARGV[5]) ? 0 : 1);
    ...
    exit(print_signature_hash(dnsmasq_signature_body(uci_settings(), uci_dnsmasq(), uci_exists("dhcp.padkap-evolution"))) ? 0 : 1);
