...
const SERVICE_NAME = getenv("PADKAP_EVOLUTION_SERVICE_NAME") || "padkap-evolution";
const STATE_UC = LIB_DIR + "/service/state.uc";
const UI_UC = LIB_DIR + "/service/ui.uc";
const STATE_DIR = getenv("PADKAP_EVOLUTION_UI_STATE_DIR") || "/var/run/padkap-evolution/ui-state";
const PENDING_RELOAD_FILE = getenv("PADKAP_EVOLUTION_PENDING_RELOAD_FILE") || "/var/run/padkap-evolution/reload.pending";
const SERVICE_ACTION_DIR = getenv("PADKAP_EVOLUTION_UI_SERVICE_ACTION_DIR") || STATE_DIR + "/service-actions";
const SERVICE_ACTION_LOCK_DIR = getenv("PADKAP_EVOLUTION_UI_SERVICE_ACTION_LOCK_DIR") || STATE_DIR + "/service-actions.lock";
const LATENCY_ACTION_DIR = getenv("PADKAP_EVOLUTION_UI_LATENCY_ACTION_DIR") || STATE_DIR + "/latency-actions";
const COMPONENT_ACTION_DIR = getenv("PADKAP_EVOLUTION_UI_COMPONENT_ACTION_DIR") || getenv("UPDATES_JOB_DIR") || "/var/run/padkap-evolution/component-actions";
const SUBSCRIPTION_ACTION_DIR = getenv("PADKAP_EVOLUTION_UI_SUBSCRIPTION_ACTION_DIR") || getenv("PADKAP_EVOLUTION_SUBSCRIPTION_UPDATE_JOB_DIR") || "/var/run/padkap-evolution/subscription-update-jobs";
const SING_BOX_VERSION_CACHE_FILE = getenv("PADKAP_EVOLUTION_UI_SING_BOX_VERSION_CACHE_FILE") || STATE_DIR + "/sing-box-version";
const SING_BOX_VERSION_CACHE_LOCK_DIR = getenv("PADKAP_EVOLUTION_UI_SING_BOX_VERSION_CACHE_LOCK_DIR") || SING_BOX_VERSION_CACHE_FILE + ".lock";
const SING_BOX_VARIANT_STATE_FILE = getenv("PADKAP_EVOLUTION_UI_SING_BOX_VARIANT_STATE_FILE") || "/etc/padkap-evolution/sing-box-variant";
const SING_BOX_BIN_PATH = getenv("PADKAP_EVOLUTION_UI_SING_BOX_BIN_PATH") || "/usr/bin/sing-box";
const SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS = getenv("PADKAP_EVOLUTION_UI_SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS") || "1";
const SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS = getenv("PADKAP_EVOLUTION_UI_SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS") || "30";
const ACTION_FINISHED_TTL_MINUTES = getenv("PADKAP_EVOLUTION_UI_ACTION_FINISHED_TTL_MINUTES") || "60";
const ACTION_ACKED_TTL_SECONDS = getenv("PADKAP_EVOLUTION_UI_ACTION_ACKED_TTL_SECONDS") || "15";
const ACTION_STALE_GRACE_SECONDS = getenv("PADKAP_EVOLUTION_UI_ACTION_STALE_GRACE_SECONDS") || "15";
const SERVICE_ACTION_TIMEOUT_SECONDS = getenv("PADKAP_EVOLUTION_UI_SERVICE_ACTION_TIMEOUT_SECONDS") || "120";
const SERVICE_ACTION_SETTLE_SECONDS = getenv("PADKAP_EVOLUTION_UI_SERVICE_ACTION_SETTLE_SECONDS") || "2";
const RUNTIME_STABLE_MIN_AGE = getenv("PADKAP_EVOLUTION_RUNTIME_STABLE_MIN_AGE") || "2";
const NFT_TABLE_NAME = getenv("NFT_TABLE_NAME") || "PadkapEvolutionTable";
const RT_TABLE_NAME = getenv("RT_TABLE_NAME") || "padkap-evolution";
...
function ui_state_json() {
    ...
    let padkap_evolution_running = arg_number(ARGV[1]);
    let padkap_evolution_enabled = arg_number(ARGV[2]);
    let padkap_evolution_status = as_string(ARGV[3]);
    let padkap_evolution_dns_configured = arg_number(ARGV[4]);
    let sing_box_running = arg_number(ARGV[5]);
    let sing_box_enabled = arg_number(ARGV[6]);
    let sing_box_status = as_string(ARGV[7]);

    if (padkap_evolution_status == "")
        padkap_evolution_status = service_status_text(padkap_evolution_running, padkap_evolution_enabled);
    ...
    write_json({
        service: {
            "padkap-evolution": {
                running: padkap_evolution_running,
                enabled: padkap_evolution_enabled,
                status: padkap_evolution_status,
                dns_configured: padkap_evolution_dns_configured
            },
            sing_box: {
...
function padkap_evolution_running() {
    return module_success(LIB_DIR + "/service/state.uc", [
...
function current_ui_state_json() {
    ...
    let padkap_evolution_is_running = padkap_evolution_running() ? 1 : 0;
    let padkap_evolution_is_enabled = service_enabled() ? 1 : 0;
    let sing_box_is_running = padkap_evolution_is_running ? 1 : (sing_box_running() ? 1 : 0);
    let sing_box_is_enabled = sing_box_enabled() ? 1 : 0;
    let padkap_evolution_status = service_status_text(padkap_evolution_is_running, padkap_evolution_is_enabled);
    ...
    if (active_action == "start")
        padkap_evolution_status = "starting";
    else if (active_action == "stop")
        padkap_evolution_status = "stopping";
    else if (active_action == "restart")
        padkap_evolution_status = "restarting";
    else if (active_action == "reload")
        padkap_evolution_status = "reloading";

    write_json({
        service: {
            "padkap-evolution": {
                running: padkap_evolution_is_running,
                enabled: padkap_evolution_is_enabled,
                status: padkap_evolution_status,
...
function service_action_reached_expected_state(action) {
    ...
    if (expected < 0)
        return false;
    return expected == 1 ? padkap_evolution_running() : !padkap_evolution_running();
}
...
function launch_worker(args) {
    ...
    let command = command_env({
        ...
        PADKAP_EVOLUTION_SERVICE_NAME: SERVICE_NAME,
        PADKAP_EVOLUTION_UI_STATE_DIR: STATE_DIR,
        PADKAP_EVOLUTION_UI_SERVICE_ACTION_DIR: SERVICE_ACTION_DIR,
        PADKAP_EVOLUTION_UI_SERVICE_ACTION_LOCK_DIR: SERVICE_ACTION_LOCK_DIR,
        PADKAP_EVOLUTION_UI_LATENCY_ACTION_DIR: LATENCY_ACTION_DIR,
        PADKAP_EVOLUTION_UI_COMPONENT_ACTION_DIR: COMPONENT_ACTION_DIR,
        PADKAP_EVOLUTION_UI_SUBSCRIPTION_ACTION_DIR: SUBSCRIPTION_ACTION_DIR,
        PADKAP_EVOLUTION_UI_SING_BOX_VERSION_CACHE_FILE: SING_BOX_VERSION_CACHE_FILE,
        PADKAP_EVOLUTION_UI_SING_BOX_VERSION_CACHE_LOCK_DIR: SING_BOX_VERSION_CACHE_LOCK_DIR,
        PADKAP_EVOLUTION_UI_SING_BOX_VARIANT_STATE_FILE: SING_BOX_VARIANT_STATE_FILE,
        PADKAP_EVOLUTION_UI_SING_BOX_BIN_PATH: SING_BOX_BIN_PATH,
        PADKAP_EVOLUTION_UI_SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS: SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS,
        PADKAP_EVOLUTION_UI_SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS: SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS,
        PADKAP_EVOLUTION_UI_ACTION_FINISHED_TTL_MINUTES: ACTION_FINISHED_TTL_MINUTES,
        PADKAP_EVOLUTION_UI_ACTION_ACKED_TTL_SECONDS: ACTION_ACKED_TTL_SECONDS,
        PADKAP_EVOLUTION_UI_ACTION_STALE_GRACE_SECONDS: ACTION_STALE_GRACE_SECONDS,
        PADKAP_EVOLUTION_UI_SERVICE_ACTION_TIMEOUT_SECONDS: SERVICE_ACTION_TIMEOUT_SECONDS,
        PADKAP_EVOLUTION_UI_SERVICE_ACTION_SETTLE_SECONDS: SERVICE_ACTION_SETTLE_SECONDS,
...
function run_pending_reload_after_service_action(action, success) {
    ...
    command_success_from_args([ "logger", "-t", SERVICE_NAME, "[info] Applying pending Padkap Evolution reload" ]);
...
function finish_service_action_after_command(action, job_id_value, status, spawn_waiter) {
    ...
    if (!service_enabled() && !padkap_evolution_running()) {
...
function service_action_worker(path, action, job_id_value, reason) {
    ...
    if (reason != "")
        push(args, reason);
    let command = "PADKAP_EVOLUTION_UI_ACTION_TRACKED=1 " + command_from_args(args) + " >/dev/null 2>&1";
