...
const START_RETRY_FILE = getenv("PADKAP_EVOLUTION_START_RETRY_FILE") || RUNTIME_STATE_DIR + "/start.retry";
const START_RETRY_PID_FILE = getenv("PADKAP_EVOLUTION_START_RETRY_PID_FILE") || RUNTIME_STATE_DIR + "/start-retry.pid";
const START_RETRY_DELAY_SECONDS = getenv("PADKAP_EVOLUTION_START_RETRY_DELAY_SECONDS") || "30";
...
const CONFIG_CHANGE_REASON = getenv("PADKAP_EVOLUTION_CONFIG_CHANGE_REASON") || "on_config_change";
...
function retry_start_on_wan_up(owner_pid) {
    ...
    command_success_from_args([ "logger", "-t", SERVICE_NAME, "[info] Retrying failed Padkap Evolution start after WAN came up" ]);
...
function handle_wan_up(owner_pid) {
    ...
    if (action == "reload") {
        ...
        command_success_from_args([ "logger", "-t", SERVICE_NAME, "[info] Reloading Padkap Evolution after monitored WAN came up" ]);
...
function ui_action_tracked() {
    return as_string(getenv("PADKAP_EVOLUTION_UI_ACTION_TRACKED") || "0") == "1";
}
...
function start_service(reason, owner_pid) {
    ...
    if (status == 0) {
        ...
        command_success_from_args([ "logger", "-t", SERVICE_NAME, "[warn] Padkap Evolution start failed; scheduled an automatic retry" ]);
    }
...
function reload_service(reason, owner_pid) {
    ...
    let status = command_status(command_from_args([ "env", "PADKAP_EVOLUTION_UI_ACTION_TRACKED=1", BIN_PATH, "reload", reason ]) + " >/dev/null 2>&1");
