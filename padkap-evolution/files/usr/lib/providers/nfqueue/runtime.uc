...
function start_runtime(cfg) {
    ...
    if (!ensure_runtime_dirs(cfg)) {
        log_message("Failed to prepare the Padkap Evolution " + cfg.status_label + " state directory in " + cfg.state_dir + ". Aborted.", "fatal");
        exit(1);
    }
...
function status_json(cfg) {
    ...
    if (configured && !provider)
        ...
        message = "external NFQUEUE rules overlap with the Padkap Evolution " + cfg.status_label + " range " + cfg.queue_base + "-" + queue_range_end(cfg);
        ...
        message = "unexpected Padkap Evolution-managed " + cfg.binary_name + " processes are running without matching action=" + cfg.action + " rules";
    else if (configured && !ready)
        message = "action=" + cfg.action + " is configured, but the Padkap Evolution-managed " + cfg.binary_name + " runtime is not ready";
    else if (standalone_conflict)
        message = "standalone " + cfg.status_label + " is active together with Padkap Evolution action=" + cfg.action + "; queues are separate, but packet-level policy overlap is possible";
