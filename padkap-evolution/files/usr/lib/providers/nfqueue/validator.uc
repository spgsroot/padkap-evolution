...
function unsupported_reason(kind, token) {
    ...
    if (token == "<HOSTLIST>" || token == "<HOSTLIST_NOAUTO>")
        return "Padkap Evolution does not expand " + action + " hostlist templates in per-rule strategies because sing-box already selects the resources before NFQUEUE.";
    ...
    if (contains([
        ...
        return "Hostname-based selection inside " + name + " is incompatible with the Padkap Evolution architecture because sing-box already chooses which resources enter action=" + action + ".";

    if (contains([ "--ipset", "--ipset-ip", "--ipset-exclude", "--ipset-exclude-ip" ], base))
        return "IP or CIDR selection inside " + name + " is incompatible with the Padkap Evolution architecture because sing-box already chooses which resources enter action=" + action + ".";

    if (base == "--qnum")
        return "The NFQUEUE number is assigned by Padkap Evolution per rule and must not be overridden in the strategy.";

    if (kind == "nfqws" && base == "--dpi-desync-fwmark")
        return "The desync fwmark is managed by Padkap Evolution for loop prevention and must not be overridden in the strategy.";

    if (kind == "nfqws2" && (base == "--fwmark" || base == "--dpi-desync-fwmark"))
        return "The nfqws2 fwmark is managed by Padkap Evolution for loop prevention and must not be overridden in the strategy.";

    if (kind == "nfqws2" && base == "--fuzz")
        return "nfqws2 fuzz mode disables normal interception and is incompatible with Padkap Evolution-managed action=zapret2 rules.";

    if (kind == "nfqws2" && token == "--intercept=0")
        return "nfqws2 interception must stay enabled for Padkap Evolution-managed action=zapret2 rules.";

    if (base == "--daemon")
        return "Padkap Evolution manages the " + name + " process lifecycle itself. The strategy must not daemonize " + name + ".";
    if (base == "--dry-run")
    ...
    for (let i = 0; i < length(tokens); ) {
        ...
        if (token_count == 0 && token_has_external_config_prefix(token))
            return validation_failure(
                "Unsupported " + label(kind) + " token '" + token + "': External " + (kind == "nfqws2" ? "nfqws2" : "nfqws") + " config files bypass Padkap Evolution validation and queue management.",
                [ token ]
            );
