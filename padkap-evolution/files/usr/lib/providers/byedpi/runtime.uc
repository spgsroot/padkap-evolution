...
const BYEDPI_STATE_DIR = getenv("BYEDPI_STATE_DIR") || "/var/run/padkap-evolution/byedpi";
...
function start_runtime() {
    ...
    if (standalone_service_enabled())
        log_message("Standalone byedpi service is enabled. Padkap Evolution manages ciadpi itself for action 'byedpi'; disable standalone byedpi autostart to avoid boot-time port conflicts.", "warn");

    if (standalone_service_running()) {
        log_message("Stopping standalone byedpi service before starting Padkap Evolution-managed ciadpi runtime", "info");
        command_success_from_args([ BYEDPI_SERVICE_INIT, "stop" ]);
        command_success_from_args([ "sleep", "1" ]);
        if (standalone_service_running()) {
            log_message("Standalone byedpi service is still running and may conflict with Padkap Evolution-managed ciadpi runtime. Aborted.", "fatal");
            exit(1);
        }
    }

    if (!ensure_runtime_dirs()) {
        log_message("Failed to prepare the Padkap Evolution ByeDPI state directory in " + BYEDPI_STATE_DIR + ". Aborted.", "fatal");
        exit(1);
    }
...
function status_json() {
    ...
    if (configured && !provider)
        ...
        message = "standalone byedpi service is active together with Padkap Evolution action=byedpi; port conflicts are possible";
    else if (configured && standalone_enabled)
        message = "standalone byedpi service autostart is enabled; disable it to avoid boot-time port conflicts with Padkap Evolution action=byedpi";
    else if (running > expected || supervisors > expected)
        message = "unexpected Padkap Evolution-managed ciadpi processes are running without matching action=byedpi rules";
    else if (configured && unstable)
        message = "Padkap Evolution-managed ciadpi has restarted after exiting; the ByeDPI strategy or traffic load may be unstable";
    else if (configured && !ready)
        message = "action=byedpi is configured, but the Padkap Evolution-managed ciadpi runtime is not ready";
