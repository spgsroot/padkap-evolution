...
const PADKAP_EVOLUTION_INTERNAL_CONFIG_TRIGGER_GUARD = getenv("PADKAP_EVOLUTION_INTERNAL_CONFIG_TRIGGER_GUARD") || "/var/run/padkap-evolution.internal-config-change";
...
function subscription_cache_paths(section) {
    return [
        ...
        PADKAP_EVOLUTION_SUBSCRIPTION_METADATA_DIR + "/" + section + ".json",
        PADKAP_EVOLUTION_SUBSCRIPTION_LINKS_DIR + "/" + section + ".json",
        PADKAP_EVOLUTION_OUTBOUND_METADATA_DIR + "/" + section + ".json",
        PADKAP_EVOLUTION_SECTION_CACHE_DIR + "/" + section + ".json",
...
// Podkop Plus -> Padkap Evolution source migration.
...
function migrate_padkap_evolution_model(model) {
    let ctx = migration_context(model);
    apply_migrations(ctx);
    return ctx;
}
...
function ensure_runtime_cache_format() {
    ...
    if (first_line(PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT_FILE) != PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT) {
        if (first_line(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) == PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT)
        ...
        ensure_runtime_dirs();
        fs.writefile(PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT_FILE, PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT + "\n");
    }

    if (first_line(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) != PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT) {
        run("rm -rf " + shell_quote(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR));
        ensure_dir(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR);
        run("chmod 700 " + shell_quote(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR) + " >/dev/null 2>&1");
        fs.writefile(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE, PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT + "\n");
        run("chmod 600 " + shell_quote(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) + " >/dev/null 2>&1");
    }
}

function remove_legacy_server_country_cache() {
    fs.unlink(PADKAP_EVOLUTION_RUNTIME_STATE_DIR + "/server-country-cache.json");
}
...
function mark_internal_config_guard() {
    ...
    if (hash == "") {
        fs.unlink(PADKAP_EVOLUTION_INTERNAL_CONFIG_TRIGGER_GUARD);
        return;
    }

    let stamp = clock();
    let tmp_path = PADKAP_EVOLUTION_INTERNAL_CONFIG_TRIGGER_GUARD + "." + stamp[0] + "." + stamp[1];
    fs.writefile(tmp_path, as_string(stamp[0]) + "\n" + hash + "\n");
    if (!fs.rename(tmp_path, PADKAP_EVOLUTION_INTERNAL_CONFIG_TRIGGER_GUARD))
        fs.unlink(tmp_path);
}
...
function migrate_model(model, source) {
    return source == "podkop"
        ...
        : migrate_padkap_evolution_model(model);
}
...
function main(argv) {
    ...
    warn("       config/migration.uc migrate-fixture <fixture.json> [padkap-evolution|podkop]\n");
    return 1;
}
...
function module_exports() {
    return {
        ...
        migrate_padkap_evolution_model: migrate_padkap_evolution_model,
