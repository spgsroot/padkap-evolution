...
function log_message(message, level) {
    ...
    run_args([ "logger", "-t", "padkap-evolution", "[" + level + "] " + as_string(message) ]);
}
...
function safe_rm_rf(path) {
    path = as_string(path);
    if (path == "" || substr(path, 0, length("/var/run/padkap-evolution/")) != "/var/run/padkap-evolution/")
        return true;
...
function validate_runtime_mark_ranges_context(context) {
    ...
    if (fakeip_mark == null || outbound_mark == null)
        fail_validation("Padkap Evolution marks contain invalid numeric constants. Aborted.");
...
function sing_box_variant_marker(ctx, value) {
    ...
    if (path == "")
        path = "/etc/padkap-evolution/sing-box-variant";
...
function sing_box_version_state(ctx) {
    ...
    if (path == "")
        path = "/etc/padkap-evolution/sing-box-version";
...
function managed_sing_box_service_script(marker) {
    ...
    if (marker == "")
        marker = "Padkap Evolution managed sing-box service for binary variants";
...
function install_managed_sing_box_service_script(ctx) {
    let stamp = clock();
    let tmp_file = sprintf("/etc/init.d/sing-box.padkap-evolution.%d.%d", stamp[0], stamp[1]);
...
function check_provider_requirements(ctx) {
    ...
    check_provider_requirement(
        ...
        "Failed to prepare the Padkap Evolution zapret state directory in " + ctx.zapret_state_dir + ". Aborted."
    );
    ...
    check_provider_requirement(
        ...
        "Failed to prepare the Padkap Evolution zapret2 state directory in " + ctx.zapret2_state_dir + ". Aborted."
    );
    ...
    check_provider_requirement(
        ...
        "Failed to prepare the Padkap Evolution ByeDPI state directory in " + ctx.byedpi_state_dir + ". Aborted."
    );
}
...
function check_runtime_requirements() {
    ...
    if (dhcp_has_https_dns_proxy_options("/etc/config/dhcp") === true)
        log_message("https-dns-proxy is enabled in DHCP config. Disable it or edit /etc/config/dhcp before starting Padkap Evolution.", "error");

    if (has_outbound_section(ctx))
        log_message("Proxy outbound configuration found", "debug");
    else
        log_message("No proxy outbound sections found. Padkap Evolution will use direct and/or provider-only routing.", "warn");
