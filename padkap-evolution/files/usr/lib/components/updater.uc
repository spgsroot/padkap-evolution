...
function padkap_evolution_release_plan(latest_version, asset_ext, i18n_required) {
    let release = object_or_empty(read_stdin_json());
    ...
    let backend = release_asset_pair(release, "padkap_evolution_" + latest_version + "." + asset_ext);
    let app = release_asset_pair(release, "luci-app-padkap-evolution_" + latest_version + "." + asset_ext);
    if (backend == null || app == null)
        exit(1);

    let i18n = { name: "", url: "" };
    if (arg_bool(i18n_required)) {
        i18n = release_asset_pair(release, "luci-i18n-padkap-evolution-ru_" + latest_version + "." + asset_ext);
        if (i18n == null)
            exit(1);
    }
...
function padkap_evolution_normalized_release_version(value) {
    return as_string(value);
}

function padkap_evolution_release_version_valid(value) {
    value = padkap_evolution_normalized_release_version(value);
    return match(value, /^[0-9]+[.][0-9]+[.][0-9]+$/) != null;
}

function padkap_evolution_release_version_parts(value) {
    let version = padkap_evolution_normalized_release_version(value);
    if (!padkap_evolution_release_version_valid(version))
        return null;
...
function padkap_evolution_release_version_compare(lhs, rhs) {
    let lhs_parts = padkap_evolution_release_version_parts(lhs);
    let rhs_parts = padkap_evolution_release_version_parts(rhs);
...
else if (mode == "padkap-evolution-release-plan")
    padkap_evolution_release_plan(ARGV[1], ARGV[2], ARGV[3]);
...
else if (mode == "padkap-evolution-release-version-valid")
    exit(padkap_evolution_release_version_valid(ARGV[1]) ? 0 : 1);
else if (mode == "padkap-evolution-release-version-compare")
    exit(padkap_evolution_release_version_compare(ARGV[1], ARGV[2]) ? 0 : 1);
