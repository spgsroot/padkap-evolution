...
const BIN_PATH = getenv("PADKAP_EVOLUTION_BIN") || constants.PADKAP_EVOLUTION_BIN || "/usr/bin/padkap-evolution";
const SERVICE_INIT = getenv("PADKAP_EVOLUTION_SERVICE_INIT") || constants.PADKAP_EVOLUTION_SERVICE_INIT || "/etc/init.d/padkap-evolution";
const PADKAP_EVOLUTION_VERSION = getenv("PADKAP_EVOLUTION_VERSION") || constants.PADKAP_EVOLUTION_VERSION || "";
const PADKAP_EVOLUTION_RELEASE_REPO = getenv("PADKAP_EVOLUTION_RELEASE_REPO") || constants.PADKAP_EVOLUTION_RELEASE_REPO || "spgsroot/padkap-evolution";
...
const SB_MANAGED_SERVICE_MARKER = getenv("SB_MANAGED_SERVICE_MARKER") || constants.SB_MANAGED_SERVICE_MARKER || "Padkap Evolution managed sing-box service for binary variants";

let tmp_dir = "";
let lock_held = false;
let padkap_evolution_was_running = false;
let padkap_evolution_stopped_for_sing_box_change = false;
...
function log_message(message, level) {
    ...
    command_success_from_args([ "logger", "-t", "padkap-evolution", "[" + level + "] " + as_string(message) ]);
}
...
function cleanup_stale_tmp_files() {
    command_success_from_args([ "find", "/tmp", "-maxdepth", "1", "-type", "d", "-name", "padkap-evolution-updates.*", "-mmin", "+" + as_string(TMP_STALE_TTL_MINUTES), "-exec", "rm", "-rf", "{}", "+" ]);
    command_success_from_args([ "find", "/tmp", "-maxdepth", "1", "-type", "f", "(", "-name", "padkap-evolution-updates-command.*", "-o", "-name", "padkap-evolution-updates-http.*", ")", "-mmin", "+" + as_string(TMP_FILE_STALE_TTL_MINUTES), "-delete" ]);
}
...
function init_tmp_dir() {
    ...
    tmp_dir = trim(command_output_from_args([ "mktemp", "-d", "/tmp/padkap-evolution-updates.XXXXXX" ]));
    if (tmp_dir == "") {
        tmp_dir = "/tmp/padkap-evolution-updates." + owner_pid();
...
function make_tmp_file(prefix) {
    init_tmp_dir();
    let base = tmp_dir != "" ? tmp_dir + "/" + as_string(prefix) + ".XXXXXX" : "/tmp/padkap-evolution-updates-" + as_string(prefix) + ".XXXXXX";
...
function restart_padkap_evolution_after_failed_sing_box_change() {
    if (!padkap_evolution_stopped_for_sing_box_change || !padkap_evolution_was_running || !file_exists(SERVICE_INIT))
        return;
    updates_log("Restarting Padkap Evolution after failed sing-box component change");
...
function action_fail(component, action, message, current_version, latest_version, status, release_url) {
    ...
    restart_padkap_evolution_after_failed_sing_box_change();
...
function run_logged(description, command) {
    ...
    if (output_file == "")
        output_file = "/tmp/padkap-evolution-updates-command." + owner_pid();
...
function latest_padkap_evolution_release_json() {
    let parts = split(PADKAP_EVOLUTION_RELEASE_REPO, "/");
    if (length(parts) != 2 || as_string(parts[0]) == "" || as_string(parts[1]) == "")
        return "";
    return fetch_github_release_json(parts[0], parts[1]);
}

function latest_padkap_evolution_version() {
    let response = latest_padkap_evolution_release_json();
    if (response == "")
        return "";
    return trim(helper_output_input(response, "object-get-default", [ "tag_name", "" ]));
}

function fetch_padkap_evolution_latest_release_metadata() {
    let response = latest_padkap_evolution_release_json();
    if (response == "")
        return "";
...
function write_padkap_evolution_latest_version_cache(value, timestamp) {
    if (as_string(value) == "")
        return;
    write_file("/tmp/padkap-evolution.latest-version.cache", as_string(value) + "\n" + as_string(timestamp) + "\n");
}
...
function clear_version_caches() {
    remove_file("/tmp/padkap-evolution.latest-version.cache");
    remove_file(SYSTEM_INFO_CACHE_FILE);
    remove_file("/tmp/padkap-evolution/system-info.json");
}
...
function install_managed_sing_box_service_script() {
    let tmp = "/etc/init.d/sing-box.padkap-evolution." + owner_pid();
...
function padkap_evolution_status_running_with_timeout() {
    init_tmp_dir();
    let output_file = make_tmp_file("padkap-evolution-status");
    if (output_file == "")
        return false;
...
function capture_padkap_evolution_running_state() {
    padkap_evolution_was_running = file_exists(BIN_PATH) && padkap_evolution_status_running_with_timeout();
}

function restart_padkap_evolution_after_successful_change() {
    if (!file_exists(SERVICE_INIT))
        return;
    if (!padkap_evolution_was_running) {
        updates_log("Padkap Evolution was not running before component change; restart skipped");
        prepare_sing_box_service_disabled();
        return;
    }
    run_logged("Restarting Padkap Evolution after successful component change", command_from_args([ SERVICE_INIT, "restart" ]));
}

function stop_padkap_evolution_before_sing_box_change() {
    if (padkap_evolution_stopped_for_sing_box_change)
        return;
    padkap_evolution_stopped_for_sing_box_change = true;

    if (padkap_evolution_was_running && file_exists(SERVICE_INIT))
        run_logged("Stopping Padkap Evolution before sing-box package change", command_from_args([ SERVICE_INIT, "stop" ]));

    if (padkap_evolution_was_running && file_exists(BIN_PATH))
        command_success_from_args([ BIN_PATH, "restore_dnsmasq" ]);

    prepare_sing_box_service_disabled();
}

function wait_padkap_evolution_running_after_sing_box_change() {
    if (!padkap_evolution_was_running)
        return true;
    ...
    let waited = 0;
    while (waited < 60) {
        if (padkap_evolution_status_running_with_timeout()) {
...
function install_zapret_like(component, action, runtime_module, resolve_fn, label) {
    ...
    restart_padkap_evolution_after_successful_change();
...
function move_file_portable(source_path, target_path) {
    ...
    let staged_path = as_string(target_path) + ".padkap-evolution-move." + owner_pid();
...
function install_staged_file(source_path, target_path, mode) {
    let staged_path = as_string(target_path) + ".padkap-evolution-new." + owner_pid();
    remove_file(staged_path);
...
function install_sing_box_extended_package(action) {
    ...
    stop_padkap_evolution_before_sing_box_change();
    prepare_sing_box_package_service_install();
    ...
    if (current_variant == "extended" || current_variant == "extended-compressed") {
        if (file_exists("/usr/bin/sing-box")) {
            backup_binary = "/usr/bin/sing-box.padkap-evolution-backup." + owner_pid();
        ...
        if (file_exists("/usr/lib/libcronet.so")) {
            cronet_touched = true;
            backup_cronet = "/usr/lib/libcronet.so.padkap-evolution-backup." + owner_pid();
    ...
    write_sing_box_variant_state("extended", new_version);
    restart_padkap_evolution_after_successful_change();
    if (!wait_padkap_evolution_running_after_sing_box_change()) {
        ...
        if (restore_sing_box_after_failed_extended_package_install(current_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, package_file, cronet_touched)) {
            remove_file(backup_binary);
            remove_file(backup_cronet);
            action_fail("sing_box", action, "sing-box-extended package was installed but Padkap Evolution did not start cleanly; previous sing-box variant was restored", current_version, latest_version);
        }
        action_fail("sing_box", action, "sing-box-extended package was installed but Padkap Evolution did not start cleanly and previous sing-box variant could not be restored", current_version, latest_version);
    }
...
function install_sing_box_extended(action, compressed) {
    ...
    if (!wait_padkap_evolution_running_after_sing_box_change()) {
        ...
        if (restore_sing_box_after_failed_extended_install(current_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, archive_file, cronet_touched)) {
            remove_file(backup_binary);
            remove_file(backup_cronet);
            action_fail("sing_box", action, label + " was installed but Padkap Evolution did not start cleanly; previous sing-box variant was restored", current_version, latest_version);
        }
        action_fail("sing_box", action, label + " was installed but Padkap Evolution did not start cleanly and previous sing-box variant could not be restored", current_version, latest_version);
    }
...
function install_package_sing_box(action, tiny) {
    ...
    if (file_exists("/usr/bin/sing-box")) {
        backup_binary = backup_on_tmpfs ? tmp_dir + "/sing-box.padkap-evolution-backup" :
            "/usr/bin/sing-box.padkap-evolution-backup." + owner_pid();
        if (!move_file_to_backup("/usr/bin/sing-box", backup_binary))
            action_fail("sing_box", action, "Failed to backup current sing-box binary", current_version, latest_version);
    }
    if (file_exists("/usr/lib/libcronet.so")) {
        cronet_touched = true;
        backup_cronet = backup_on_tmpfs ? tmp_dir + "/libcronet.so.padkap-evolution-backup" :
            "/usr/lib/libcronet.so.padkap-evolution-backup." + owner_pid();
        if (!move_file_to_backup("/usr/lib/libcronet.so", backup_cronet)) {
    ...
    restart_padkap_evolution_after_successful_change();
    if (!wait_padkap_evolution_running_after_sing_box_change())
        fail_package_sing_box_install(action, tiny, "was installed, but Padkap Evolution did not start cleanly", new_version, latest_version,
            package_name, previous_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, cronet_touched);
    remove_file(backup_binary);
    remove_file(backup_cronet);
    clear_version_caches();
...
function check_padkap_evolution() {
    let metadata = fetch_padkap_evolution_latest_release_metadata();
    ...
    if (latest_version == "unknown")
        action_fail("padkap-evolution", "check_update", "Failed to check Padkap Evolution updates", PADKAP_EVOLUTION_VERSION, latest_version);

    write_padkap_evolution_latest_version_cache(latest_version, now_seconds());
    if (!helper_success("padkap-evolution-release-version-valid", [ PADKAP_EVOLUTION_VERSION ])) {
        updates_log("Padkap Evolution current version is not a release version (" + PADKAP_EVOLUTION_VERSION + ")");
        action_success("padkap-evolution", "check_update", "Installed version is newer than release", PADKAP_EVOLUTION_VERSION, latest_version, 0, "dev", release_url);
    }

    let compare = trim(helper_output("padkap-evolution-release-version-compare", [ PADKAP_EVOLUTION_VERSION, latest_version ]));
    if (compare == "")
        action_fail("padkap-evolution", "check_update", "Failed to compare Padkap Evolution versions", PADKAP_EVOLUTION_VERSION, latest_version);
    let status = status_from_compare(int(compare));
    if (status == "")
        action_fail("padkap-evolution", "check_update", "Failed to compare Padkap Evolution versions", PADKAP_EVOLUTION_VERSION, latest_version);
    if (status == "latest") {
        updates_log("Padkap Evolution is already up to date (" + PADKAP_EVOLUTION_VERSION + ")");
        action_success("padkap-evolution", "check_update", "Latest version is installed", PADKAP_EVOLUTION_VERSION, latest_version, 0, status, release_url);
    }
    if (status == "outdated") {
        updates_log("Padkap Evolution update found: " + PADKAP_EVOLUTION_VERSION + " -> " + latest_version);
        action_success("padkap-evolution", "check_update", "Update is available", PADKAP_EVOLUTION_VERSION, latest_version, 0, status, release_url);
    }
    updates_log("Padkap Evolution installed version is newer than upstream release: " + PADKAP_EVOLUTION_VERSION + " -> " + latest_version);
    action_success("padkap-evolution", "check_update", "Installed version is newer than release", PADKAP_EVOLUTION_VERSION, latest_version, 0, status, release_url);
}

function resolve_padkap_evolution_release(latest_version) {
    let release_json = latest_padkap_evolution_release_json();
    if (release_json == "")
        return null;
    let asset_ext = is_apk() ? "apk" : "ipk";
    let i18n_required = pkg_is_installed("luci-i18n-padkap-evolution-ru") ? "1" : "0";
    let plan = trim(helper_output_input(release_json, "padkap-evolution-release-plan", [ latest_version, asset_ext, i18n_required ]));
...
function install_padkap_evolution() {
    let latest_version = latest_padkap_evolution_version();
    if (latest_version == "")
        latest_version = "unknown";
    if (latest_version == "unknown")
        action_fail("padkap-evolution", "install", "Failed to resolve Padkap Evolution release", PADKAP_EVOLUTION_VERSION, latest_version);

    write_padkap_evolution_latest_version_cache(latest_version, now_seconds());
    init_tmp_dir() || action_fail("padkap-evolution", "install", "Failed to create temporary directory", PADKAP_EVOLUTION_VERSION, latest_version);
    updates_log("Resolving Padkap Evolution release " + latest_version + " packages");
    let release = resolve_padkap_evolution_release(latest_version);
    if (release == null)
        action_fail("padkap-evolution", "install", "Failed to resolve Padkap Evolution release packages", PADKAP_EVOLUTION_VERSION, latest_version);
    ...
    if (!download_with_retry(release.backend_url, backend_file, release.backend_name) ||
        ...
        action_fail("padkap-evolution", "install", "Failed to download Padkap Evolution release packages", PADKAP_EVOLUTION_VERSION, latest_version);

    if (!run_logged("Installing LuCI app package " + release.app_name, pkg_install_files_command([ app_file ])))
        action_fail("padkap-evolution", "install", "Failed to install LuCI app package", PADKAP_EVOLUTION_VERSION, latest_version);
    if (i18n_file != "" && !run_logged("Installing LuCI Russian i18n package " + release.i18n_name, pkg_install_files_command([ i18n_file ])))
        action_fail("padkap-evolution", "install", "Failed to install LuCI Russian i18n package", PADKAP_EVOLUTION_VERSION, latest_version);
    if (!run_logged("Installing Padkap Evolution package " + release.backend_name, pkg_install_files_command([ backend_file ])))
        action_fail("padkap-evolution", "install", "Failed to install Padkap Evolution package", PADKAP_EVOLUTION_VERSION, latest_version);
    ...
    restart_padkap_evolution_after_successful_change();
    clear_version_caches();
    let new_version = installed_package_version("padkap-evolution");
    if (new_version == "")
        new_version = latest_version;
    updates_log("Padkap Evolution updated to " + new_version);
    action_success("padkap-evolution", "install", "Padkap Evolution has been installed", new_version, latest_version, 1, "latest", release.release_url);
}
...
function normalize_component_name(component) {
    ...
    if (component == "sing-box" || component == "singbox")
        return "sing_box";
    if (component == "padkap-evolution")
        return "padkap-evolution";
    return component;
}
...
function component_action(component, action) {
    ...
    capture_padkap_evolution_running_state();

    if (component == "padkap-evolution" && action == "check_update")
        check_padkap_evolution();
    else if (component == "padkap-evolution" && action == "install")
        install_padkap_evolution();
...
else if (mode == "latest-padkap-evolution-release-json")
    print(latest_padkap_evolution_release_json());
else if (mode == "latest-padkap-evolution-version")
    print(latest_padkap_evolution_version(), "\n");
else if (mode == "padkap-evolution-release-metadata")
    print(fetch_padkap_evolution_latest_release_metadata(), "\n");
else {
    warn("Usage: components/action.uc <component-action|latest-padkap-evolution-version|padkap-evolution-release-metadata> ...\n");
    exit(1);
}
