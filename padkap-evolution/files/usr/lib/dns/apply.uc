...
function log(message, level) {
    ...
    run("logger -t " + shell_quote("padkap-evolution") + " " + shell_quote("[" + level + "] " + as_string(message)));
}
...
function dnsmasq_legacy_instance_exists() {
    return uci_exists("dhcp.padkap-evolution");
}
...
function dnsmasq_default_has_padkap_evolution_dns() {
    return list_has(dnsmasq_default_servers(), SB_DNS_INBOUND_ADDRESS);
}

function dnsmasq_has_padkap_evolution_dns() {
    return dnsmasq_default_has_padkap_evolution_dns() || dnsmasq_legacy_instance_exists();
}

function dnsmasq_has_padkap_evolution_managed_state() {
    return uci_get("dhcp.@dnsmasq[0].padkap_evolution_server") != "" ||
        uci_get("dhcp.@dnsmasq[0].padkap_evolution_noresolv") != "" ||
        uci_get("dhcp.@dnsmasq[0].padkap_evolution_cachesize") != "" ||
        uci_get("dhcp.@dnsmasq[0].padkap_evolution_notinterface") != "" ||
...
function dnsmasq_default_config_is_complete() {
    return dnsmasq_default_has_padkap_evolution_dns() &&
...
function dnsmasq_legacy_interfaces() {
    let legacy_dnsmasq_section = "padkap-evolution";
...
function backup_dnsmasq_server_list() {
    if (uci_get("dhcp.@dnsmasq[0].padkap_evolution_server") != "")
        return;

    for (let server in words(dnsmasq_default_servers())) {
        if (server != SB_DNS_INBOUND_ADDRESS)
            uci_add_list("dhcp.@dnsmasq[0].padkap_evolution_server", server);
    }
}
...
function dnsmasq_cleanup_legacy_instance() {
    ...
    uci_delete("dhcp.padkap-evolution");

    let backup_notinterfaces = uci_get("dhcp.@dnsmasq[0].padkap_evolution_notinterface");
    if (backup_notinterfaces != "") {
        ...
        uci_delete("dhcp.@dnsmasq[0].padkap_evolution_notinterface");
        return;
    }
...
function dnsmasq_configure_default_instance() {
    let default_has_padkap_evolution_dns = dnsmasq_default_has_padkap_evolution_dns();

    backup_dnsmasq_server_list();
    if (!default_has_padkap_evolution_dns) {
        backup_dnsmasq_config_option("noresolv", "padkap_evolution_noresolv");
        backup_dnsmasq_config_option("cachesize", "padkap_evolution_cachesize");
    }
...
function dnsmasq_restore_default_instance() {
    ...
    let backup_servers = uci_get("dhcp.@dnsmasq[0].padkap_evolution_server");
    ...
    if (backup_servers != "") {
        ...
        uci_delete("dhcp.@dnsmasq[0].padkap_evolution_server");
    }
    else {
    ...
    let noresolv = uci_get("dhcp.@dnsmasq[0].padkap_evolution_noresolv");
    if (noresolv != "")
        restore_dnsmasq_config_option("noresolv", "padkap_evolution_noresolv", "");
    else if (managed_global_dns)
        uci_set("dhcp.@dnsmasq[0].noresolv", "0");

    let cachesize = uci_get("dhcp.@dnsmasq[0].padkap_evolution_cachesize");
    if (cachesize != "")
        restore_dnsmasq_config_option("cachesize", "padkap_evolution_cachesize", "");
    else if (managed_global_dns)
...
function dnsmasq_configure(force) {
    ...
    if (as_string(force) != "force" && uci_get(CONFIG_NAME + ".settings.shutdown_correctly") == "0") {
        if (dnsmasq_default_config_is_complete()) {
            log("Previous Padkap Evolution shutdown was unclean; dnsmasq already points to sing-box", "info");
            return true;
        }
        log("Previous Padkap Evolution shutdown was unclean and dnsmasq is not ready; applying Padkap Evolution DNS settings", "info");
    }
...
function dnsmasq_restore(force, quiet) {
    ...
    if (as_string(force) != "force" && uci_get(CONFIG_NAME + ".settings.shutdown_correctly") == "1") {
        if (!dnsmasq_has_padkap_evolution_dns()) {
            log("dnsmasq already uses non-Padkap Evolution DNS settings; restore is not required", "info");
            return true;
        }
        log("Padkap Evolution DNS settings are still present after a clean shutdown; restoring DNS settings in dnsmasq", "info");
    }
...
function failsafe_restore() {
    ...
    if (dnsmasq_management_disabled()) {
        if (!dnsmasq_has_padkap_evolution_managed_state()) {
            log("DNS rollback skipped: dont_touch_dhcp is enabled and no Padkap Evolution dnsmasq changes were found", "info");
            return true;
        }

        log("Rolling back previous Padkap Evolution dnsmasq changes because dont_touch_dhcp is enabled", "warn");
    }
    else {
        log("Rolling back Padkap Evolution DNS changes in dnsmasq", "warn");
    }
...
else if (mode == "has-padkap-evolution-dns")
    exit(dnsmasq_has_padkap_evolution_dns() ? 0 : 1);
else if (mode == "has-managed-state")
    exit(dnsmasq_has_padkap_evolution_managed_state() ? 0 : 1);
else if (mode == "default-config-complete")
    exit(dnsmasq_default_config_is_complete() ? 0 : 1);

warn("Usage: dns/apply.uc <configure|restore|failsafe-restore|has-padkap-evolution-dns|has-managed-state|default-config-complete>\n");
exit(1);
