...
const CONFIG_NAME = "padkap-evolution";
...
function subscription_group_outbound(outbound) {
    ...
    return (t == "selector" || t == "urltest") && internal_flag(outbound.__padkap_evolution_allow_group);
}

function subscription_urltest_group_outbound(outbound) {
    if (type(outbound) != "object")
        return false;
    return as_string(outbound.type || "") == "urltest" && internal_flag(outbound.__padkap_evolution_allow_group);
}
...
function subscription_hidden_outbound(outbound, refs, hide_urltest_group_outbounds, hide_detour_outbounds) {
    ...
    if (hidden_by_detour && hide_detour_outbounds !== false)
        return true;
    return internal_flag(outbound.__padkap_evolution_hidden) && !hidden_by_urltest && !hidden_by_detour;
}
...
function copy_subscription_outbound(outbound, new_tag) {
    ...
    for (let key, value in outbound) {
        if (key != "tag" && key != "remark" && key != "share_link" &&
            key != "__padkap_evolution_hidden" && key != "__padkap_evolution_allow_group")
            copy[key] = value;
    }
