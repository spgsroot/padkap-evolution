...
const PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT = int(getenv("PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT") || "8");
...
let section_cache_dir = PADKAP_EVOLUTION_SECTION_CACHE_DIR;
...
function legacy_metadata_path(section_name) {
    return PADKAP_EVOLUTION_SUBSCRIPTION_METADATA_DIR + "/" + section_name + ".json";
}

function persistent_source_path(source_section, suffix) {
    return PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR + "/" + source_section + suffix;
}
...
function new_section_state(section_name) {
    return {
        version: PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT,
