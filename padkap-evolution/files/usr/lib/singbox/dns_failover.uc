...
const STATE_FILE = getenv("PADKAP_EVOLUTION_DNS_FAILOVER_STATE_FILE") || RUNTIME_STATE_DIR + "/dns-failover.json";
const PID_FILE = getenv("PADKAP_EVOLUTION_DNS_FAILOVER_PID_FILE") || RUNTIME_STATE_DIR + "/dns-failover.pid";
const DNS_FAILOVER_UC = getenv("PADKAP_EVOLUTION_DNS_FAILOVER_UC") || LIB_DIR + "/singbox/dns_failover.uc";
const SERVICE_BIN = getenv("PADKAP_EVOLUTION_BIN") || "/usr/bin/padkap-evolution";
...
function log_message(message, level) {
    command_success_from_args([ "logger", "-t", "padkap-evolution", "[" + as_string(level || "info") + "] DNS failover: " + as_string(message) ]);
}
