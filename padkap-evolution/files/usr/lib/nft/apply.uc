...
const DNS_SOURCE_SET = "padkap_evolution_dns_sources";
const DNS_SOURCE6_SET = "padkap_evolution_dns_sources6";
...
function log_debug(message) {
    run_args([ "logger", "-t", "padkap-evolution", "[debug] " + as_string(message) ]);
}

function log_fatal(message) {
    run_args([ "logger", "-t", "padkap-evolution", "[fatal] " + as_string(message) ]);
}
...
function section_priority_prefix(section) {
    return "padkap_evolution_rule_" + as_string(section[".name"]);
}
...
function nft_add_global_proxy_marking_rules(table, interface_set, fakeip_mark) {
    return nft_add_rule(table, "mangle", [ "iifname", "@" + as_string(interface_set), "meta", "l4proto", "tcp", "meta", "mark", "set", fakeip_mark, "counter" ]) &&
...
function nft_create_runtime_base(table, localv4_set, common_set, port_set, ip_port_set, interface_set, source_interfaces, fakeip_mark, outbound_mark, fakeip_range, tproxy_port, exclude_ntp, localv6_set, common6_set, ip_port6_set, fakeip6_range, tproxy6_address, global_proxy, block_doh) {
    localv6_set = default_arg(localv6_set, "localv6");
    common6_set = default_arg(common6_set, "padkap_evolution_subnets6");
    ip_port6_set = default_arg(ip_port6_set, "padkap_evolution_ip6_ports");
    fakeip6_range = default_arg(fakeip6_range, "fc00::/18");
...
function uci_global_proxy_active() {
    for (let section in uci_sections("section")) {
        let action = option(section, "action", "");
        if (bool_option(section, "enabled", true) && bool_option(section, "global_proxy", false) && connections.is_connections_action(action))
            return true;
    }
    return false;
}
...
function nft_add_inline_ip_cidr_matchers(csv, ports_csv, table, common_set, ip_port_set, chunk_size_text, common6_set, ip_port6_set) {
    ...
    if (as_string(ports_csv) != "")
        return nft_add_csv_chunks_to_family_sets(csv, table, ip_port_set, default_arg(ip_port6_set, "padkap_evolution_ip6_ports"), "ip-port-from-ip", ports_csv, chunk_size_text);

    return nft_add_csv_chunks_to_family_sets(csv, table, common_set, default_arg(common6_set, "padkap_evolution_subnets6"), "ips", "", chunk_size_text);
}
...
function nft_rule_signature_body(body, section) {
    ...
    body = signature_add_value(body, "rule." + section_name + ".global_proxy", bool_option(section, "global_proxy", false) ? "1" : "0");
    if (action == "dns") {
...
function nft_add_extracted_ruleset_subnets(unscoped_path, scoped_path, label, table, common_set, ip_port_set, chunk_size_text, common6_set, ip_port6_set) {
    ...
    if (file_nonempty(unscoped_path)) {
        if (!nft_add_file_chunks_to_family_sets(unscoped_path, table, common_set, default_arg(common6_set, "padkap_evolution_subnets6"), "ips", "", chunk_size_text))
            return false;
        has_entries = true;
    }

    if (file_nonempty(scoped_path)) {
        if (!nft_add_file_chunks_to_family_sets(scoped_path, table, ip_port_set, default_arg(ip_port6_set, "padkap_evolution_ip6_ports"), "ip-ports", "", chunk_size_text))
            return false;
        has_entries = true;
    }

    if (!has_entries)
        run_args([ "logger", "-t", "padkap-evolution", "[warn] " + as_string(label) + " has no ip_cidr entries for nftables" ]);
