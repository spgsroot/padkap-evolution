...
function padkap_evolution_config_masked_line(line) {
...
function padkap_evolution_config_masked(path) {
    let data = fs.readfile(path);
    if (data == null)
        exit(1);
    ...
    for (let i = 0; i < length(lines); i++) {
        ...
        let masked = padkap_evolution_config_masked_line(line);
        print(masked, "\n");
...
function render_global_system_info() {
    let value = object_or_empty(read_stdin_json());
    let padkap_evolution_version = object_value(value, "padkap_evolution_version") || "unknown";
    ...
    print_line("\ud83d\udd73\ufe0f Padkap Evolution:   " + padkap_evolution_version);
...
function nft_ruleset_other_mark_lines(table_name) {
    ...
    let in_padkap_evolution_table = false;

    for (let line in split(read_stdin(), "\n")) {
        line = as_string(line);
        if (index(line, "table inet " + table_name) >= 0) {
            in_padkap_evolution_table = true;
            continue;
        }

        if (match(line, /^table/) != null)
            in_padkap_evolution_table = false;

        if (!in_padkap_evolution_table && (index(line, "mark set") >= 0 || index(line, "meta mark") >= 0))
            print_line(line);
    }
}
        ...
        if (line_contains(line, needle))
        {
            if (needle == "sing-box" && is_udp_unsupported_outbound_log(line)) {
                if (!udp_http_notice_emitted) {
                    push(filtered, "UDP traffic through HTTP outbounds is not supported by sing-box; repeated UDP warnings for HTTP outbounds are hidden by Padkap Evolution.");
...
function render_padkap_evolution_logs() {
    let lines = split(read_stdin(), "\n");
    let filtered = [];
    let start = -1;
    let udp_http_notice_emitted = false;
    ...
    for (let line in lines) {
        if (line == "")
            continue;
        if (!line_contains(line, "padkap-evolution") && !line_contains(line, "sing-box"))
            continue;
        ...
        if (line_contains(line, "padkap-evolution") && line_contains(line, "Starting Padkap Evolution"))
    ...
    print_line("No 'Starting PadkapEvolution' message found, showing last 100 lines");
...
function system_info_cache_valid(path, padkap_evolution_version, luci_app_version, ttl, now) {
    ...
    return cache.padkap_evolution_version == padkap_evolution_version && cache.luci_app_version == luci_app_version;
}

function system_info_json() {
    write_json({
        padkap_evolution_version: as_string(ARGV[1]),
        padkap_evolution_latest_version: as_string(ARGV[2]),
...
else if (mode == "padkap-evolution-config-masked")
    padkap_evolution_config_masked(ARGV[1]);
...
else if (mode == "padkap-evolution-logs")
    render_padkap_evolution_logs();
