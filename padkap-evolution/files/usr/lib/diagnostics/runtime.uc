...
const CONFIG_NAME = getenv("PADKAP_EVOLUTION_CONFIG_NAME") || constants.PADKAP_EVOLUTION_CONFIG_NAME || "padkap-evolution";
...
const PADKAP_EVOLUTION_CONFIG = getenv("PADKAP_EVOLUTION_CONFIG") || constants.PADKAP_EVOLUTION_CONFIG || "/etc/config/" + CONFIG_NAME;
const PADKAP_EVOLUTION_SERVICE_NAME = getenv("PADKAP_EVOLUTION_SERVICE_NAME") || constants.PADKAP_EVOLUTION_SERVICE_NAME || "padkap-evolution";
...
const PADKAP_EVOLUTION_LUCI_VIEW_DIR = getenv("PADKAP_EVOLUTION_LUCI_VIEW_DIR") || constants.PADKAP_EVOLUTION_LUCI_VIEW_DIR || "/www/luci-static/resources/view/padkap-evolution";
...
const SYSTEM_INFO_CACHE_TTL = int(getenv("PADKAP_EVOLUTION_SYSTEM_INFO_CACHE_TTL") || "3600");
...
const RT_TABLE_NAME = getenv("RT_TABLE_NAME") || constants.RT_TABLE_NAME || "padkap-evolution";
const NFT_TABLE_NAME = getenv("NFT_TABLE_NAME") || constants.NFT_TABLE_NAME || "PadkapEvolutionTable";
const NFT_FAKEIP_MARK = getenv("NFT_FAKEIP_MARK") || constants.NFT_FAKEIP_MARK || "0x04000000";
const NFT_COMMON_SET_NAME = getenv("NFT_COMMON_SET_NAME") || constants.NFT_COMMON_SET_NAME || "padkap_evolution_subnets";
const NFT_PORT_SET_NAME = getenv("NFT_PORT_SET_NAME") || constants.NFT_PORT_SET_NAME || "padkap_evolution_ports";
const NFT_IP_PORT_SET_NAME = getenv("NFT_IP_PORT_SET_NAME") || constants.NFT_IP_PORT_SET_NAME || "padkap_evolution_ip_ports";
const NFT_INTERFACE_SET_NAME = getenv("NFT_INTERFACE_SET_NAME") || constants.NFT_INTERFACE_SET_NAME || "padkap_evolution_interfaces";
const NFT_DISCORD_SET_NAME = getenv("NFT_DISCORD_SET_NAME") || constants.NFT_DISCORD_SET_NAME || "padkap_evolution_discord_subnets";
...
const SB_VARIANT_STATE_FILE = getenv("SB_VARIANT_STATE_FILE") || constants.SB_VARIANT_STATE_FILE || "/etc/padkap-evolution/sing-box-variant";
const SING_BOX_BIN_PATH = getenv("PADKAP_EVOLUTION_DIAGNOSTICS_SING_BOX_BIN_PATH") || "/usr/bin/sing-box";
...
function check_logs() {
    ...
    let rendered = status_capture([ "padkap-evolution-logs" ], command_output_from_args([ "logread" ]));
    if (rendered.output != "")
...
function padkap_evolution_logs_fixture() {
    let rendered = status_capture([ "padkap-evolution-logs" ], read_stdin());
    if (rendered.output != "")
        print(rendered.output);
...
function show_config(visibility) {
    visibility = as_string(visibility || "masked");
    if (!file_exists(PADKAP_EVOLUTION_CONFIG)) {
        nolog("Configuration file not found");
        return 1;
    }
    if (visibility == "raw")
        print(as_string(fs.readfile(PADKAP_EVOLUTION_CONFIG)));
    else
        print(status_output([ "padkap-evolution-config-masked", PADKAP_EVOLUTION_CONFIG ], null));
    return 0;
}

function show_version() {
    print(PADKAP_EVOLUTION_VERSION, "\n");
    return 0;
}
...
function get_luci_app_version() {
    let path = PADKAP_EVOLUTION_LUCI_VIEW_DIR + "/main.js";
    ...
    for (let line in split(as_string(data), "\n")) {
        ...
        if (matched != null && matched[1] == "PADKAP_EVOLUTION_LUCI_APP_VERSION")
...
function system_info_cache_is_valid() {
    ...
    if (now > 0 && generated_at > 0 && SYSTEM_INFO_CACHE_TTL > 0 && now - generated_at >= SYSTEM_INFO_CACHE_TTL)
        return false;
    return cache.padkap_evolution_version == PADKAP_EVOLUTION_VERSION && cache.luci_app_version == get_luci_app_version();
}
...
function build_system_info() {
    let padkap_evolution_latest_version = first_line_value("/tmp/padkap-evolution.latest-version.cache", "unknown");
    ...
    return {
        padkap_evolution_version: PADKAP_EVOLUTION_VERSION,
        padkap_evolution_latest_version: padkap_evolution_latest_version || "unknown",
        luci_app_version,
        sing_box_version,
...
function neutralize_zapret_defaults() {
    log_message("Standalone zapret is not neutralized automatically; Padkap Evolution uses /opt/zapret/nfq/nfqws as an external provider and manages only its own NFQUEUE range.", "info");
    return 0;
}
...
function dnsmasq_has_padkap_evolution_dns() {
    return module_success(DNS_APPLY_UC, [ "has-padkap-evolution-dns" ]);
}
...
function get_sing_box_status() {
    ...
    let dns_configured = dnsmasq_has_padkap_evolution_dns() ? 1 : 0;
...
function get_status() {
    let running = module_success(SERVICE_STATE_UC, [
        "padkap-evolution-stably-running", RT_TABLE_NAME, NFT_TABLE_NAME, NFT_FAKEIP_MARK, RUNTIME_STABLE_MIN_AGE
    ]) ? 1 : 0;
    let enabled = file_executable("/etc/rc.d/S99" + PADKAP_EVOLUTION_SERVICE_NAME) ? 1 : 0;
    let dns_configured = dnsmasq_has_padkap_evolution_dns() ? 1 : 0;
    write_service_status(running, enabled, dns_configured);
    return 0;
}
...
function global_check(arg1, arg2) {
    ...
    print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    print_global("📄 Padkap Evolution config");
    ...
    if (file_executable("/etc/init.d/zapret") && command_success_from_args([ "/etc/init.d/zapret", "status" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret service is active. Padkap Evolution uses separate queues, but packet-level policy overlap is possible.");
    }
    else if (file_executable("/etc/init.d/zapret") && command_success_from_args([ "/etc/init.d/zapret", "enabled" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret autostart is enabled. Padkap Evolution will not modify /etc/config/zapret.");
    }

    if (file_executable("/etc/init.d/zapret2") && command_success_from_args([ "/etc/init.d/zapret2", "status" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret2 service is active. Padkap Evolution uses separate queues, but packet-level policy overlap is possible.");
    }
    else if (file_executable("/etc/init.d/zapret2") && command_success_from_args([ "/etc/init.d/zapret2", "enabled" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret2 autostart is enabled. Padkap Evolution will not modify /etc/config/zapret2.");
    }
...
else if (mode == "padkap-evolution-logs-fixture")
    exit(padkap_evolution_logs_fixture());
