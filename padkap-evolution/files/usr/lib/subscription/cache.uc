...
const PADKAP_EVOLUTION_RUNTIME_STATE_DIR = getenv("PADKAP_EVOLUTION_RUNTIME_STATE_DIR") || "/var/run/padkap-evolution";
const PADKAP_EVOLUTION_SUBSCRIPTION_UPDATE_STATE_DIR = getenv("PADKAP_EVOLUTION_SUBSCRIPTION_UPDATE_STATE_DIR") || PADKAP_EVOLUTION_RUNTIME_STATE_DIR + "/subscription-update";
const PADKAP_EVOLUTION_SUBSCRIPTION_LINKS_DIR = getenv("PADKAP_EVOLUTION_SUBSCRIPTION_LINKS_DIR") || PADKAP_EVOLUTION_RUNTIME_STATE_DIR + "/subscription-links";
const PADKAP_EVOLUTION_SUBSCRIPTION_METADATA_DIR = getenv("PADKAP_EVOLUTION_SUBSCRIPTION_METADATA_DIR") || PADKAP_EVOLUTION_RUNTIME_STATE_DIR + "/subscription-metadata";
const PADKAP_EVOLUTION_OUTBOUND_METADATA_DIR = getenv("PADKAP_EVOLUTION_OUTBOUND_METADATA_DIR") || PADKAP_EVOLUTION_RUNTIME_STATE_DIR + "/outbound-metadata";
const PADKAP_EVOLUTION_SECTION_CACHE_DIR = getenv("PADKAP_EVOLUTION_SECTION_CACHE_DIR") || PADKAP_EVOLUTION_RUNTIME_STATE_DIR + "/section-cache";
const PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT_FILE = getenv("PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT_FILE") || PADKAP_EVOLUTION_RUNTIME_STATE_DIR + "/cache-format";
const PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT = getenv("PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT") || "8";
const PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR = getenv("PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR") || "/etc/padkap-evolution/subscription-cache";
const PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE = getenv("PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE") || PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR + "/cache-format";
const PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT = getenv("PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT") || "7";
const PADKAP_EVOLUTION_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE = getenv("PADKAP_EVOLUTION_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE") || PADKAP_EVOLUTION_RUNTIME_STATE_DIR + "/subscription-bootstrap-retry.pid";
const PADKAP_EVOLUTION_SUBSCRIPTION_UPDATE_LOCK_DIR = getenv("PADKAP_EVOLUTION_SUBSCRIPTION_UPDATE_LOCK_DIR") || PADKAP_EVOLUTION_RUNTIME_STATE_DIR + "/subscription-update.lock";
const PADKAP_EVOLUTION_PENDING_RELOAD_FILE = getenv("PADKAP_EVOLUTION_PENDING_RELOAD_FILE") || PADKAP_EVOLUTION_RUNTIME_STATE_DIR + "/reload.pending";
const PADKAP_EVOLUTION_SERVICE_INIT = getenv("PADKAP_EVOLUTION_SERVICE_INIT") || "/etc/init.d/padkap-evolution";
...
const SB_VARIANT_STATE_FILE = getenv("SB_VARIANT_STATE_FILE") || "/etc/padkap-evolution/sing-box-variant";
const SB_VERSION_STATE_FILE = getenv("SB_VERSION_STATE_FILE") || "/etc/padkap-evolution/sing-box-version";
...
function subscription_metadata_path(section) {
    ...
    if (!cache_section_is_safe(section))
        return "";
    return PADKAP_EVOLUTION_SUBSCRIPTION_METADATA_DIR + "/" + section + ".json";
}
...
function outbound_metadata_path(section) {
    ...
    if (!cache_section_is_safe(section))
        return "";
    return PADKAP_EVOLUTION_OUTBOUND_METADATA_DIR + "/" + section + ".json";
}
...
function ensure_runtime_dirs() {
    ...
    ensure_dir(PADKAP_EVOLUTION_RUNTIME_STATE_DIR);
    ensure_dir(PADKAP_EVOLUTION_SUBSCRIPTION_UPDATE_STATE_DIR);
    ensure_dir(PADKAP_EVOLUTION_SUBSCRIPTION_METADATA_DIR);
    ensure_dir(PADKAP_EVOLUTION_OUTBOUND_METADATA_DIR);
    ensure_dir(PADKAP_EVOLUTION_SECTION_CACHE_DIR);
}

function clear_subscription_runtime_cache() {
    run_silent("rm -rf " +
        shell_quote(TMP_SUBSCRIPTION_FOLDER) + " " +
        shell_quote(PADKAP_EVOLUTION_SUBSCRIPTION_LINKS_DIR) + " " +
        shell_quote(PADKAP_EVOLUTION_SUBSCRIPTION_METADATA_DIR) + " " +
        shell_quote(PADKAP_EVOLUTION_OUTBOUND_METADATA_DIR) + " " +
        shell_quote(PADKAP_EVOLUTION_SECTION_CACHE_DIR));
}

function ensure_runtime_cache_format() {
    ensure_dir(PADKAP_EVOLUTION_RUNTIME_STATE_DIR);

    if (file_first_line_value(PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT_FILE) != PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT) {
        log_message("Runtime subscription cache format changed; clearing old subscription cache", "info");
        if (file_first_line_value(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) == PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT)
            subscription_share_link.populate_subscription_dir(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR);
        clear_subscription_runtime_cache();
        ensure_runtime_dirs();
        write_file(PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT_FILE, PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT + "\n");
    }

    if (file_first_line_value(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) != PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT) {
        run_silent("rm -rf " + shell_quote(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR));
        ensure_dir(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR);
        chmod_path(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR, "700");
        write_file(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE, PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT + "\n");
        chmod_path(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE, "600");
    }
}
...
function section_current_usable_cache_by_name(sections, section, default_user_agent) {
    return section_current_usable_cache(find_section(sections, section), TMP_SUBSCRIPTION_FOLDER, PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR, default_user_agent);
}
...
function cache_candidate_paths() {
    ...
    for (let dir in [
        ...
        PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR,
        PADKAP_EVOLUTION_SECTION_CACHE_DIR,
        PADKAP_EVOLUTION_SUBSCRIPTION_LINKS_DIR,
        PADKAP_EVOLUTION_SUBSCRIPTION_METADATA_DIR,
        PADKAP_EVOLUTION_OUTBOUND_METADATA_DIR
    ]) {
...
function persistent_metadata_path(source_section) {
    return PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR + "/" + as_string(source_section) + ".metadata.json";
}
...
function persist_subscription_cache(source_section, subscription_json_path, subscription_url, effective_user_agent, effective_hwid, metadata_path) {
    ...
    let persistent_json = source_json_path(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR, source_section);
    let persistent_url = source_url_path(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR, source_section);
    let persistent_user_agent = source_user_agent_path(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR, source_section);
    let persistent_hwid = source_hwid_path(PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR, source_section);
...
function append_available_cached_metadata(array_path, section, source_index, source_section) {
    ...
    append_persistent_source_metadata(array_path, source_index, source_section) ||
        append_cached_metadata(array_path, PADKAP_EVOLUTION_SECTION_CACHE_DIR, section, PADKAP_EVOLUTION_SUBSCRIPTION_METADATA_DIR + "/" + section + ".json", source_index, source_section);
...
function subscription_update_timestamp_path(section_name_value) {
    return PADKAP_EVOLUTION_SUBSCRIPTION_UPDATE_STATE_DIR + "/" + as_string(section_name_value) + ".timestamp";
}
...
function finalize_subscription_section_metadata(section_name_value, metadata_tmpfile, superseded) {
    ...
    if (!superseded) {
        if (length(array_or_empty(read_json(metadata_tmpfile))) > 0)
            write_subscription_metadata(PADKAP_EVOLUTION_SECTION_CACHE_DIR, PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT, section_name_value, metadata_tmpfile);
        else
            write_subscription_metadata(PADKAP_EVOLUTION_SECTION_CACHE_DIR, PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT, section_name_value, "");
    }
...
function subscription_update_selected_source(sections, section_name_value, source_index_value, force) {
    ...
    if (update_result == 0 || update_result == 2)
        write_source_metadata(
            PADKAP_EVOLUTION_SECTION_CACHE_DIR,
            PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT,
            section_name_value,
            source_index,
            source_section,
            metadata_output_path,
            PADKAP_EVOLUTION_SUBSCRIPTION_METADATA_DIR + "/" + section_name_value + ".json"
        );
...
function prepare_subscription_caches(phase, already_prepared, no_refresh) {
    ...
    if (prepared_runtime_cache_should_skip(sections, PADKAP_EVOLUTION_SECTION_CACHE_DIR, phase, already_prepared)) {
        print("\n");
        return 0;
    }
...
function mark_pending_subscription_recovery_reload() {
    state_ucode_status([ "mark-pending-reload", PADKAP_EVOLUTION_PENDING_RELOAD_FILE, "subscription_deferred_recovery" ]);
}

function trigger_subscription_recovery_reload(worker) {
    if (worker)
        command_status_from_args([ PADKAP_EVOLUTION_SERVICE_INIT, "reload", "subscription_deferred_recovery" ]);
    else
...
function worker_env() {
    return {
        ...
        TMP_RULESET_FOLDER,
        TMP_SUBSCRIPTION_FOLDER,
        PADKAP_EVOLUTION_RUNTIME_STATE_DIR,
        PADKAP_EVOLUTION_SUBSCRIPTION_UPDATE_STATE_DIR,
        PADKAP_EVOLUTION_SUBSCRIPTION_LINKS_DIR,
        PADKAP_EVOLUTION_SUBSCRIPTION_METADATA_DIR,
        PADKAP_EVOLUTION_OUTBOUND_METADATA_DIR,
        PADKAP_EVOLUTION_SECTION_CACHE_DIR,
        PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT_FILE,
        PADKAP_EVOLUTION_RUNTIME_CACHE_FORMAT,
        PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_DIR,
        PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE,
        PADKAP_EVOLUTION_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT,
        PADKAP_EVOLUTION_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE,
        PADKAP_EVOLUTION_SUBSCRIPTION_UPDATE_LOCK_DIR,
        PADKAP_EVOLUTION_PENDING_RELOAD_FILE,
        PADKAP_EVOLUTION_SERVICE_INIT,
...
function start_deferred_subscription_bootstrap_retry_worker(deferred_sections) {
    ...
    ensure_runtime_dirs();
    let existing_pid = trim(file_first_line_value(PADKAP_EVOLUTION_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE));
    ...
    if (pid != "")
        write_file(PADKAP_EVOLUTION_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE, pid + "\n");
...
function stop_deferred_subscription_bootstrap_retry_worker() {
    let pid = trim(file_first_line_value(PADKAP_EVOLUTION_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE));
    if (pid_running(pid)) {
        command_success_from_args([ "kill", pid ]);
        log_message("Stopped subscription bootstrap retry worker", "info");
    }
    unlink_path(PADKAP_EVOLUTION_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE);
}
...
function run_deferred_subscription_bootstrap(deferred_sections) {
    ...
    if (result.recovered != "") {
        log_message("Recovered deferred subscription rule(s): " + result.recovered + "; scheduling Padkap Evolution reload", "info");
...
function deferred_subscription_bootstrap_retry_worker(remaining_sections) {
    ...
    while (remaining_sections != "") {
        ...
        ensure_runtime_dirs();
        if (!state_ucode_status([ "acquire-runtime-dir-lock", PADKAP_EVOLUTION_SUBSCRIPTION_UPDATE_LOCK_DIR, current_pid() ])) {
        ...
        state_ucode_status([ "release-runtime-dir-lock", PADKAP_EVOLUTION_SUBSCRIPTION_UPDATE_LOCK_DIR ]);

        if (result.recovered != "") {
            log_message("Recovered deferred subscription rule(s): " + result.recovered + "; reloading Padkap Evolution", "info");
