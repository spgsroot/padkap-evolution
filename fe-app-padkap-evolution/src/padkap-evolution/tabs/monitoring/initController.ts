...
function buildRouteDisplayNames(sections: PadkapEvolution.ConfigSection[]) {
  ...
  if (serviceAvailability === 'stopped') {
    container.replaceChildren(
      renderConnectionsTable([], {
        text: _(
          'Padkap Evolution service is stopped. Start the service to display connections.',
        ),
      }),
    );
    return;
...
async function closeConnection(connectionId: string) {
  ...
  try {
    const response =
      await PadkapEvolutionShellMethods.closeClashApiConnection(connectionId);
...
async function closeAllConnections() {
  ...
  try {
    const response =
      await PadkapEvolutionShellMethods.closeAllClashApiConnections();
...
async function loadRouteDisplayNames() {
  try {
    buildRouteDisplayNames(
      await CustomPadkapEvolutionMethods.getConfigSections(),
    );
  } catch (error) {
...
async function pollConnectionsSnapshot() {
  ...
  try {
    const response = await PadkapEvolutionShellMethods.getClashApiConnections();
  ...
  serviceStateUnsubscribe = subscribeRuntimeUiState((uiState) => {
    ...
    setServiceAvailability(
      getServiceAvailability({
        loading: false,
        failed: false,
        running: uiState.service["padkap-evolution"].running,
      }),
    );
  });
}
