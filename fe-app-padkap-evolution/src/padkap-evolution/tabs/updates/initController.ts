...
import { PADKAP_EVOLUTION_ACTION_PROVIDERS_AVAILABILITY_EVENT } from '../../../constants';
...
type UpdateStatus =
  StoreType['updatesChecks'][PadkapEvolution.ComponentName]['status'];
...
let componentUpdateCheckCacheSnapshot: PadkapEvolution.ComponentUpdateCheckCache | null =
  null;
let componentUpdateCheckCachePromise: Promise<PadkapEvolution.ComponentUpdateCheckCache> | null =
  null;
...
function shouldShowInstallAfterCheck(component: PadkapEvolution.ComponentName) {
  const status = getVisibleCheckResult(component)?.status;

  return status === 'outdated' || status === 'dev';
}

function getVisibleCheckResult(component: PadkapEvolution.ComponentName) {
  if (
    !shouldExposeCheckResults({
...
function getLatestVersion(component: PadkapEvolution.ComponentName) {
...
function getGitHubReleaseUrl(component: PadkapEvolution.ComponentName) {
  const checkResult = getVisibleCheckResult(component);
...
function isServiceRuntimeActionLoading() {
  ...
  return (
    ...
    isServiceTransitionStatus(
      state.servicesInfoWidget.data.padkapEvolutionStatus,
    )
  );
}
...
function resetCheckResult(component: PadkapEvolution.ComponentName) {
  setCheckResult(component, null, '');
}

function applyCachedCheckResults(
  results: PadkapEvolution.ComponentActionResult[],
) {
...
function loadComponentUpdateCheckCache({ force = false } = {}) {
  ...
  const promise = PadkapEvolutionShellMethods.componentUpdateCheckCache()
    .then((response) =>
      response.success
        ? response.data
        : ({
            enabled: false,
            results: [],
          } satisfies PadkapEvolution.ComponentUpdateCheckCache),
    )
    .then((cache) => {
...
async function ackComponentActionJob(jobId: string) {
  try {
    const response = await PadkapEvolutionShellMethods.uiActionAck(
      'component',
      jobId,
    );
...
function getExpectedLatestVersionForAction(button: ComponentActionButton) {
  if (button.component !== 'padkap-evolution' || button.action !== 'install') {
    return undefined;
  }
...
function notifyActionProvidersAvailabilityChanged(
  ...
  window.dispatchEvent(
    new CustomEvent(PADKAP_EVOLUTION_ACTION_PROVIDERS_AVAILABILITY_EVENT, {
      detail: {
...
function reloadPageAfterPadkapEvolutionUpdate() {
  window.setTimeout(() => {
    window.location.reload();
  }, 1200);
}

function patchSystemInfoAfterMutation(
  result: PadkapEvolution.ComponentActionResult,
) {
  const systemInfo = store.get().diagnosticsSystemInfo;
  ...
  if (result.component === 'padkap-evolution' && result.action === 'install') {
    nextSystemInfo.padkap_evolution_version = version;
  }
...
async function applyCompletedComponentAction({
  ...
  if (result.component === 'padkap-evolution' && result.action === 'install') {
    ...
    if (notify) {
      reloadPageAfterPadkapEvolutionUpdate();
    }
    return;
  }
...
async function completeComponentActionJob(
  key: UpdatesActionKey,
  jobId: string,
  response: PadkapEvolution.MethodResponse<PadkapEvolution.ComponentActionResult>,
) {
  if (pageUnloading) {
    setActionLoading(key, false);
    return;
...
async function followComponentActionState(
  state: PadkapEvolution.ComponentActionResult,
) {
  ...
  try {
    const response = state.running
      ? await PadkapEvolutionShellMethods.waitComponentActionJob(
          jobId,
          state.component,
          state.action,
...
function handleComponentUiState(uiState: PadkapEvolution.UiState) {
...
async function handleComponentAction(button: ComponentActionButton) {
  ...
  try {
    const startResponse =
      await PadkapEvolutionShellMethods.componentActionStart(
        button.component,
        button.action,
      );
    ...
    const response = await PadkapEvolutionShellMethods.waitComponentActionJob(
      jobId,
      button.component,
      button.action,
...
function getComponentCards(): ComponentCard[] {
  ...
  const padkapEvolutionActions = getInstalledUpdateActions(
    'padkap-evolution',
    'padkapEvolutionCheck',
    'padkapEvolutionInstall',
  );
  ...
  return [
    {
      component: 'padkap-evolution',
      column: 0,
      title: 'Padkap Evolution',
      version: systemInfoLoading
        ? _('Loading...')
        : normalizeCompiledVersion(systemInfo.padkap_evolution_version),
      latestVersion: getLatestVersion('padkap-evolution'),
      releaseUrl: getGitHubReleaseUrl('padkap-evolution'),
      actions: padkapEvolutionActions,
    },
    {
      component: 'sing_box',
      column: 0,
...
function applyComponentUpdateCheckCache(
  componentUpdateCheckCache: PadkapEvolution.ComponentUpdateCheckCache,
) {
