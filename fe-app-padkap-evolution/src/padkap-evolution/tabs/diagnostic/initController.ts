...
import { PADKAP_EVOLUTION_LUCI_APP_VERSION } from '../../../constants';
...
function isMutatingServiceActionLoading() {
  return (
    ...
    isServiceTransitionStatus(
      store.get().servicesInfoWidget.data.padkapEvolutionStatus,
    )
  );
}

function getPadkapEvolutionStatusText(running: boolean, enabled: boolean) {
  if (running) {
...
function setDisplayedPadkapEvolutionRunning(running: boolean) {
  const servicesInfoWidget = store.get().servicesInfoWidget;
  const enabled = Boolean(servicesInfoWidget.data.padkapEvolutionEnabled);
  ...
  store.set({
    servicesInfoWidget: {
      ...
      data: {
        ...
        padkapEvolutionRunning: running ? 1 : 0,
        padkapEvolutionStatus: getPadkapEvolutionStatusText(running, enabled),
      },
    },
  });
}
...
async function waitForPadkapEvolutionRunningState(expectedRunning: boolean) {
  ...
  while (Date.now() - startedAt < SERVICE_ACTION_STATUS_TIMEOUT_MS) {
    ...
    const padkapEvolutionRunning = Boolean(
      store.get().servicesInfoWidget.data.padkapEvolutionRunning,
    );

    if (padkapEvolutionRunning === expectedRunning) {
      return true;
    }
...
async function followServiceActionState(
  state: PadkapEvolution.ServiceActionState,
) {
  ...
  try {
    if (state.running) {
      await PadkapEvolutionShellMethods.waitServiceActionJob(jobId);
    }
  } catch (error) {
    ...
    await refreshDiagnosticServicesInfo({ force: true, allowInactive: true });
    void PadkapEvolutionShellMethods.uiActionAck('service', jobId);
    followedServiceActionJobs.delete(jobId);
    resetDiagnosticsChecks();
  }
}

function followServiceActionsFromUiState(uiState?: PadkapEvolution.UiState) {
  if (!uiState) {
    return;
  }
...
async function fetchDiagnosticsProviderInfo({
  ...
  try {
    ...
    const [zapretRuntime, zapret2Runtime, byedpiRuntime, inboundsConfig] =
      await Promise.all([
        PadkapEvolutionShellMethods.checkZapretRuntime(),
        PadkapEvolutionShellMethods.checkZapret2Runtime(),
        PadkapEvolutionShellMethods.checkByedpiRuntime(),
        PadkapEvolutionShellMethods.checkInboundsConfig(),
      ]);
...
function renderDiagnosticRunActionWidget() {
  ...
  const servicesInfoWidget = store.get().servicesInfoWidget;
  const padkapEvolutionRunning = Boolean(
    servicesInfoWidget.data.padkapEvolutionRunning,
  );
...
async function handleServiceRuntimeAction({
  ...
  if (optimisticRunning !== undefined) {
    setDisplayedPadkapEvolutionRunning(optimisticRunning);
  }

  try {
    const startResponse =
      await PadkapEvolutionShellMethods.serviceActionStart(action);
    ...
    await waitForPadkapEvolutionRunningState(expectedRunning);
  } catch (e) {
...
async function handleEnable() {
  ...
  try {
    await PadkapEvolutionShellMethods.enable();
  } catch (e) {
...
async function handleDisable() {
  ...
  try {
    await PadkapEvolutionShellMethods.disable();
  } catch (e) {
...
async function handleShowGlobalCheck() {
  ...
  try {
    const globalCheck = await PadkapEvolutionShellMethods.globalCheck(false);
...
async function handleViewLogs() {
  ...
  try {
    const viewLogs = await PadkapEvolutionShellMethods.checkLogs();

    if (viewLogs.success) {
      const getLatestLogs = async () => {
        const latestLogs = await PadkapEvolutionShellMethods.checkLogs();
...
async function handleShowSingBoxConfig() {
  ...
  try {
    const showSingBoxConfig =
      await PadkapEvolutionShellMethods.showSingBoxConfig(false);
...
function renderDiagnosticAvailableActionsWidget() {
  ...
  const padkapEvolutionEnabled = Boolean(
    servicesInfoWidget.data.padkapEvolutionEnabled,
  );
  const padkapEvolutionRunning = Boolean(
    servicesInfoWidget.data.padkapEvolutionRunning,
  );
  const serviceTransition = getServiceTransition(
    servicesInfoWidget.data.padkapEvolutionStatus,
  );
  ...
  const renderedActions = renderAvailableActions({
    ...
    enable: {
      ...
      visible: !padkapEvolutionEnabled,
...
function renderDiagnosticSystemInfoWidget() {
  ...
  const items = [
    {
      key: 'Padkap Evolution',
      value: normalizeCompiledVersion(
        diagnosticsSystemInfo.padkap_evolution_version,
      ),
    },
    {
      key: 'Luci App',
      value: normalizeCompiledVersion(PADKAP_EVOLUTION_LUCI_APP_VERSION),
    },
    {
      key: 'Sing-box',
