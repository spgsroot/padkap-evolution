...
export async function runByedpiCheck() {
  ...
  const byedpiStatus = await PadkapEvolutionShellMethods.getByedpiStatus();
  ...
  const padkapEvolutionRuntimeReady =
    !hasByedpiRules ||
  ...
  const items: Array<IDiagnosticsChecksItem> = [
    ...
    {
      state:
        unexpectedRuntime || !padkapEvolutionRuntimeReady
          ? 'error'
          : runtimeUnstable
            ? 'warning'
            : 'success',
      key: hasByedpiRules
        ? runtimeUnstable
          ? _('Padkap Evolution-managed ciadpi runtime has restarted')
          : padkapEvolutionRuntimeReady
            ? _('Padkap Evolution-managed ciadpi runtime is ready')
            : _('Padkap Evolution-managed ciadpi runtime is not ready')
        : unexpectedRuntime
          ? _('Unexpected Padkap Evolution-managed ciadpi runtime is running')
          : _('Padkap Evolution-managed ciadpi runtime is not running'),
    ...
    },
    {
      ...
      key: standaloneServiceRunning
        ? hasByedpiRules
          ? _(
              'Standalone ByeDPI is active together with Padkap Evolution ByeDPI rules',
            )
