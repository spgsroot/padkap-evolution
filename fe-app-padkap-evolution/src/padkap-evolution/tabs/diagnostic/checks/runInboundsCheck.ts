...
function serverPrefix(item: PadkapEvolution.InboundCheckItem) {
  return `${item.label}:`;
}

function formatListen(item: PadkapEvolution.InboundCheckItem) {
  return `${item.listen}:${Number(item.listen_port || 0)} [${item.required_proto}]`;
}

function getPortConflictItem(
  item: PadkapEvolution.InboundCheckItem,
...
export async function runInboundsCheck() {
  ...
  const inboundsChecks = await PadkapEvolutionShellMethods.checkInbounds();
