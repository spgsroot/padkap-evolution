...
export async function runZapret2Check() {
  ...
  const zapret2Status = await PadkapEvolutionShellMethods.getZapret2Status();
  ...
  const items: Array<IDiagnosticsChecksItem> = [
    ...
    {
      state:
        unexpectedRuntime || !padkapEvolutionRuntimeReady ? 'error' : 'success',
      key: hasZapret2Rules
        ? padkapEvolutionRuntimeReady
          ? _('Padkap Evolution-managed nfqws2 runtime is ready')
          : _('Padkap Evolution-managed nfqws2 runtime is not ready')
        : unexpectedRuntime
          ? _('Unexpected Padkap Evolution-managed nfqws2 runtime is running')
          : _('Padkap Evolution-managed nfqws2 runtime is not running'),
    ...
    },
    {
      ...
      key: standaloneServiceRunning
        ? hasZapret2Rules
          ? _(
              'Standalone Zapret2 is active together with Padkap Evolution Zapret2 rules',
            )
