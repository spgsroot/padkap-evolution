...
export async function runZapretCheck() {
  ...
  const zapretStatus = await PadkapEvolutionShellMethods.getZapretStatus();
  ...
  const items: Array<IDiagnosticsChecksItem> = [
    ...
    },
    {
      ...
      key: hasZapretRules
        ? padkapEvolutionRuntimeReady
          ? _('Padkap Evolution-managed nfqws runtime is ready')
          : _('Padkap Evolution-managed nfqws runtime is not ready')
        : unexpectedRuntime
          ? _('Unexpected Padkap Evolution-managed nfqws runtime is running')
          : _('Padkap Evolution-managed nfqws runtime is not running'),
    ...
    },
    {
      ...
      key: standaloneServiceRunning
        ? hasZapretRules
          ? _(
              'Standalone Zapret is active together with Padkap Evolution Zapret rules',
            )
