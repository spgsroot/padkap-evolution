...
const PADKAP_EVOLUTION_MASK_AFTER_TOKEN = [
  'option proxy_string',
  'option hwid',
...
const PADKAP_EVOLUTION_MASK_AFTER_TOKEN_SPACE = [
...
function maskGlobalCheckLine(line: string) {
  ...
  for (const token of PADKAP_EVOLUTION_MASK_AFTER_TOKEN) {
    maskedLine = maskAfterToken(maskedLine, token);
  }

  for (const token of PADKAP_EVOLUTION_MASK_AFTER_TOKEN_SPACE) {
