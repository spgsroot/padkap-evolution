...
async function fetchDashboardSectionsOnce(mountId: number) {
  ...
  try {
    const { data, success } =
      await CustomPadkapEvolutionMethods.getDashboardSections();
...
function setLatencyFetching(
  ...
  fetching: boolean,
  local = false,
  progress?: PadkapEvolution.LatencyActionProgress,
) {
  if (local || !fetching) {
...
async function completeSubscriptionUpdateJob(
  jobId: string,
  sectionName: string,
  response: PadkapEvolution.MethodResponse<PadkapEvolution.SubscriptionUpdateJobState>,
) {
  if (pageUnloading) {
  ...
  if (jobId && response.success) {
    void PadkapEvolutionShellMethods.uiActionAck('subscription', jobId);
  }
...
async function followSubscriptionUpdateState(
  state: PadkapEvolution.SubscriptionUpdateJobState,
) {
  ...
  try {
    const response = state.running
      ? await PadkapEvolutionShellMethods.waitSubscriptionUpdateJob(jobId)
      : ({
          success: true,
          data: state,
...
async function completeLatencyTestJob(jobId: string, sectionName: string) {
  ...
  if (jobId) {
    void PadkapEvolutionShellMethods.uiActionAck('latency', jobId);
  }

  void fetchDashboardSections({ force: true });
}

async function followLatencyTestState(
  state: PadkapEvolution.LatencyActionState,
) {
  const jobId = state.job_id;
  const sectionName = state.section || '';
  ...
  try {
    if (state.running) {
      await PadkapEvolutionShellMethods.waitLatencyTestJob(jobId);
    }
...
function followDashboardActionsFromUiState(uiState: PadkapEvolution.UiState) {
  for (const state of uiState.actions.subscription || []) {
    if (state.running || (state.job_id && state.section)) {
      ...
      void PadkapEvolutionShellMethods.uiActionAck(
        'subscription',
        state.job_id,
      );
    }
  ...
  for (const state of uiState.actions.latency || []) {
    if (state.running || (state.job_id && state.section)) {
      ...
      void PadkapEvolutionShellMethods.uiActionAck('latency', state.job_id);
    }
  }
}
...
function getDashboardServiceAvailability() {
  ...
  return getServiceAvailability({
    ...
    running: service.data.padkapEvolutionRunning,
  });
}
...
async function handleChooseOutbound(
  ...
  try {
    await PadkapEvolutionShellMethods.setClashApiGroupProxy(selector, tag);
...
function getInitialLatencyProgress(
  ...
  tag: string,
): PadkapEvolution.LatencyActionProgress | undefined {
...
async function handleTestLatency(
  ...
  try {
    const startResponse = await PadkapEvolutionShellMethods.latencyTestStart(
      latencyType,
      sectionName,
      tag,
      timeout,
...
function handleCopyOutbound(outbound: PadkapEvolution.Outbound) {
...
function renderDetailsMemberName(member: PadkapEvolution.UrlTestMember) {
...
function renderUrlTestSelectedValue(info: PadkapEvolution.UrlTestInfo) {
...
function renderUrlTestInfoModal(outbound: PadkapEvolution.Outbound) {
...
function handleShowUrlTestInfo(outbound: PadkapEvolution.Outbound) {
...
function renderPrioritySelectedValue(info: PadkapEvolution.PriorityInfo) {
  const selectedMember = info.outbounds.find((member) => member.selected);
  const selectedName =
    selectedMember?.displayName || info.selectedName || info.selectedCode || '';
  const name = formatUrlTestModalValue(selectedName);
...
function renderPriorityMemberName(member: PadkapEvolution.PriorityMember) {
...
function renderPriorityInfoModal(outbound: PadkapEvolution.Outbound) {
...
function handleShowPriorityInfo(outbound: PadkapEvolution.Outbound) {
...
async function handleUpdateSubscription(
  section: PadkapEvolution.OutboundGroup,
) {
  if (
  ...
  try {
    const startResponse =
      await PadkapEvolutionShellMethods.subscriptionUpdateStart(
        section.sectionName,
      );
    ...
    items: [
      {
        ...
        value: servicesInfoWidget.data.padkapEvolutionRunning
          ? _('✔ Running')
          : _('✘ Stopped'),
        attributes: {
          class: servicesInfoWidget.data.padkapEvolutionRunning
