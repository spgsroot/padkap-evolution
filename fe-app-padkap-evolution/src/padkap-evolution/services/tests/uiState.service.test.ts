...
function createUiState(
  actions: Partial<PadkapEvolution.UiState['actions']> = {},
  capabilities: Partial<PadkapEvolution.UiState['capabilities']> = {},
): PadkapEvolution.UiState {
  return {
    service: {
      'padkap-evolution': {
        running: 0,
...
describe('applyUiStateToStore', () => {
  ...
  it('applies service, capability, and running action state before first render', () => {
    ...
    expect(state.servicesInfoWidget).toMatchObject({
      ...
      failed: false,
      data: {
        padkapEvolutionEnabled: 1,
        padkapEvolutionRunning: 0,
        padkapEvolutionStatus: 'starting',
      },
    });
