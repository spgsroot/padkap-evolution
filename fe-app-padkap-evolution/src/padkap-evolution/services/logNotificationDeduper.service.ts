const LOG_NOTIFICATION_STORAGE_KEY =
  'padkap-evolution:shown-log-error-notifications:v1';
const MAX_STORED_LOG_NOTIFICATIONS = 500;

export type PadkapEvolutionLogNotification =
...
export function getPadkapEvolutionLogNotification(
  line: string,
): PadkapEvolutionLogNotification | null {
  if (isErrorLogLine(line)) {
    return { kind: 'error', line };
  }

  const update = line.match(
    /\[component-update\]\s+(padkap-evolution|sing_box|zapret|zapret2|byedpi)\s+(\S+)/i,
  );
...
export class LogNotificationDeduper {
  ...
  shouldNotify(line: string) {
    if (!getPadkapEvolutionLogNotification(line)) {
      return false;
    }
