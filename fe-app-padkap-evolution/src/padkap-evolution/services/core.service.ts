...
import {
  ...
  PadkapEvolutionLogNotification,
...
function componentDisplayName(component: string) {
  const names: Record<string, string> = {
    'padkap-evolution': 'Padkap Evolution',
    sing_box: 'sing-box',
    zapret: 'Zapret',
    zapret2: 'Zapret2',
    byedpi: 'ByeDPI',
...
function showLogNotification(notification: PadkapEvolutionLogNotification) {
...
export function coreService(options: CoreServiceOptions = {}) {
  ...
  watcher.init(
    async () => {
      const logs = await PadkapEvolutionShellMethods.checkLogs();
    ...
    },
    {
      ...
      onNewLog: (line) => {
        if (logNotificationDeduper.shouldNotify(line)) {
          const notification = getPadkapEvolutionLogNotification(line);
          if (notification) {
