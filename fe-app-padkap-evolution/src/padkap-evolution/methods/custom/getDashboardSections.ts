...
import { PadkapEvolutionShellMethods } from '../shell';

interface IGetDashboardSectionsResponse {
  success: boolean;
  data: PadkapEvolution.OutboundGroup[];
}
...
type DashboardSectionCache = {
  ...
  outboundMetadata?: PadkapEvolution.GetOutboundMetadata;
...
const DASHBOARD_SECTION_CACHE_DIR = '/var/run/padkap-evolution/section-cache';
const CLASH_API_FETCH_TIMEOUT_MS = 5000;

function getDisplayName(section: PadkapEvolution.ConfigSection) {
  return section.label || section['.name'];
}

function getSettingsSection(configSections: PadkapEvolution.ConfigSection[]) {
  return configSections.find((section) => section['.type'] === 'settings');
}

function getClashApiSecret(configSections: PadkapEvolution.ConfigSection[]) {
...
async function getClashApiProxies(
  configSections: PadkapEvolution.ConfigSection[],
): Promise<PadkapEvolution.MethodResponse<ClashAPI.Proxies>> {
  ...
  return PadkapEvolutionShellMethods.getClashApiProxies();
}
...
function ownedChildSections(
  parent: PadkapEvolution.ConfigSection,
  children: PadkapEvolution.ConfigSection[],
) {
  return children.filter(
    (section): section is PadkapEvolution.ConfigSection =>
...
function childSectionsByOwner(
  children: PadkapEvolution.ConfigSection[],
  ownerKey: keyof PadkapEvolution.ConfigSection,
  ownerValue: string,
) {
...
function hydrateConfigSections(
  configSections: PadkapEvolution.ConfigSection[],
) {
  ...
  return configSections.map((section) => {
    ...
    const next: PadkapEvolution.ConfigSection = { ...section };
...
function getManualProxyLinks(section: PadkapEvolution.ConfigSection) {
  return getListValues(section.selector_proxy_links);
}

function getConnectionInterfaces(section: PadkapEvolution.ConfigSection) {
  const values = getListValues(section.interfaces);
  return values.length ? values : getListValues(section.interface);
}

function getJsonOutbounds(section: PadkapEvolution.ConfigSection) {
...
function hasSubscriptionSources(section: PadkapEvolution.ConfigSection) {
  return getSubscriptionSourceCount(section) > 0;
}

function getSubscriptionSourceCount(section: PadkapEvolution.ConfigSection) {
  return getListValues(section.subscription_urls).length;
}

function shouldSortByLatency(section: PadkapEvolution.ConfigSection) {
  return section.sort_by_latency === '1';
}

function hasConfiguredUrlTestList(section: PadkapEvolution.ConfigSection) {
  return getListValues(section.urltests).length > 0;
}

function hasConfiguredPriorityList(section: PadkapEvolution.ConfigSection) {
  return getListValues(section.priority_groups).length > 0;
}

function getUrlTestIds(section: PadkapEvolution.ConfigSection) {
...
function isUrlTestEnabled(section: PadkapEvolution.ConfigSection) {
  return getUrlTestIds(section).length > 0;
}

function shouldUseProxyGroup(section: PadkapEvolution.ConfigSection) {
  return (
...
function getSectionProxyConfigType(section: PadkapEvolution.ConfigSection) {
...
function getJsonOutboundDisplayName(section: PadkapEvolution.ConfigSection) {
  try {
...
function buildManualLinkByCode(section: PadkapEvolution.ConfigSection) {
...
function isSelectorOutbound(outbound: PadkapEvolution.Outbound) {
...
function sortOutboundsForDashboard(
  outbounds: PadkapEvolution.Outbound[],
  options: {
    pinnedCode?: string;
    pinnedCodes?: string[];
...
function sortUrlTestMembers(outbounds: PadkapEvolution.UrlTestMember[]) {
  return outbounds
    .map((outbound, index) => ({ outbound, index }))
    .sort((left, right) => {
...
function getUrlTestDisplayName(
  section: PadkapEvolution.ConfigSection,
  id: string,
  settings: ItemSettings | undefined,
) {
...
function getPriorityGroupIds(section: PadkapEvolution.ConfigSection) {
  return getListValues(section.priority_groups);
}

function getPriorityConfigs(
  section: PadkapEvolution.ConfigSection,
): PriorityConfig[] {
...
function metadataMatchesCurrentSource(
  sectionName: string,
  sourceCount: number,
  metadata: PadkapEvolution.SubscriptionMetadata,
) {
  const legacyMetadata = metadata as PadkapEvolution.SubscriptionMetadata & {
    source_index?: number;
