...
describe('PadkapEvolutionShellMethods.componentAction', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it('does not fail Padkap Evolution self-update when status polling disappears after package replacement', async () => {
    ...
    const responsePromise = PadkapEvolutionShellMethods.waitComponentActionJob(
      'job-1',
      'padkap-evolution',
      'install',
      '0.7.17.11',
    ...
    await expect(responsePromise).resolves.toEqual({
      ...
      data: {
        ...
        action: 'install',
        message: 'Padkap Evolution has been installed',
  ...
  it('returns the backend component action start error message', async () => {
    ...
    await expect(
      PadkapEvolutionShellMethods.componentActionStart('zapret', 'install'),
    ).resolves.toEqual({
      success: false,
