...
const targetRoot = path.resolve(__dirname, '../luci-app-padkap-evolution/po');
...
async function main() {
    ...
    for (const file of files) {
        ...
        if (file === 'padkapEvolution.pot') {
            const potTarget = path.join(targetRoot, 'templates', 'padkapEvolution.pot');
        ...
        const match = file.match(/^padkap-evolution\.([a-zA-Z_]+)\.po$/);
        if (match) {
            const lang = match[1];
            const poTarget = path.join(targetRoot, lang, 'padkapEvolution.po');
