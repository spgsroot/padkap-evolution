...
assert_eq "*/30 * * * * /usr/bin/padkap-evolution list_update_if_due # list" \
  "$(updates_ucode list-update-cron-job 30m /usr/bin/padkap-evolution '# list')" \
...
assert_eq "0 */2 * * * /usr/bin/padkap-evolution subscription_update_if_due # subscription" \
  "$(updates_ucode subscription-update-cron-job 7200 /usr/bin/padkap-evolution '# subscription')" \
...
if updates_ucode list-update-cron-job invalid /usr/bin/padkap-evolution '# list' >/dev/null 2>&1; then
  fail "invalid list update interval should fail"
fi
if updates_ucode subscription-update-cron-job 0 /usr/bin/padkap-evolution '# subscription' >/dev/null 2>&1; then
...
assert_eq $'list\t*/30 * * * * /usr/bin/padkap-evolution list_update_if_due # list\nsubscription-error\tbad_sub\tbad\nsubscription\t*/45 * * * * /usr/bin/padkap-evolution subscription_update_if_due # subscription\ncomponent\t0 */2 * * * /usr/bin/padkap-evolution component_updates_if_due # component' \
  "$(updates_ucode cron-refresh-plan-fixture "$WORK_DIR/cron-plan.json" /usr/bin/padkap-evolution '# list' '# subscription' '# component')" \
  "cron refresh plan"
...
assert_eq "list-disabled" \
  "$(updates_ucode cron-refresh-plan-fixture "$WORK_DIR/cron-plan-list-disabled.json" /usr/bin/padkap-evolution '# list' '# subscription' '# component')" \
...
status=0
invalid_plan="$(updates_ucode cron-refresh-plan-fixture "$WORK_DIR/cron-plan-invalid-list.json" /usr/bin/padkap-evolution '# list' '# subscription' '# component')" || status="$?"
...
0 1 * * * /bin/true # keep
* * * * * /usr/bin/padkap-evolution list_update_if_due # list
* * * * * /usr/bin/padkap-evolution subscription_update_if_due # subscription
* * * * * /usr/bin/padkap-evolution component_updates_if_due # component
CRON

updates_ucode refresh-cron-fixture "$WORK_DIR/cron-plan.json" "$WORK_DIR/existing.cron" /usr/bin/padkap-evolution '# list' '# subscription' '# component' >"$WORK_DIR/cron-apply.json"
node - "$WORK_DIR/cron-apply.json" <<'JS'
const fs = require("fs");
const value = JSON.parse(fs.readFileSync(process.argv[2], "utf8"));
const expected = [
  "0 1 * * * /bin/true # keep",
  "*/30 * * * * /usr/bin/padkap-evolution list_update_if_due # list",
  "*/45 * * * * /usr/bin/padkap-evolution subscription_update_if_due # subscription",
  "0 */2 * * * /usr/bin/padkap-evolution component_updates_if_due # component",
  ""
].join("\n");
...
if (!messages.includes("The cron job removed") ||
    !messages.includes("The cron job has been created: */30 * * * * /usr/bin/padkap-evolution list_update_if_due # list") ||
    !messages.includes("The subscription cron job has been created: */45 * * * * /usr/bin/padkap-evolution subscription_update_if_due # subscription") ||
    !messages.includes("The component update check cron job has been created: 0 */2 * * * /usr/bin/padkap-evolution component_updates_if_due # component")) {
...
status=0
updates_ucode refresh-cron-fixture "$WORK_DIR/cron-plan-invalid-list.json" "$WORK_DIR/existing.cron" /usr/bin/padkap-evolution '# list' '# subscription' '# component' >"$WORK_DIR/cron-apply-invalid.json" || status="$?"
assert_eq 1 "$status" "invalid cron apply status"
node - "$WORK_DIR/cron-apply-invalid.json" <<'JS'
const fs = require("fs");
const value = JSON.parse(fs.readFileSync(process.argv[2], "utf8"));
if (value.crontab !== "0 1 * * * /bin/true # keep\n") {
  console.error("invalid plan should remove old padkap-evolution cron jobs only", JSON.stringify(value.crontab));
  process.exit(1);
}
