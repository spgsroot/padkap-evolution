...
RUNTIME_STATE="$ROOT_DIR/padkap-evolution/files/usr/lib/runtime_state.sh"
...
grep -Fq 'runtime_state.sh' "$PADKAP_EVOLUTION_BIN" &&
  fail "padkap-evolution must not source runtime_state.sh"

service_predicates='process_age_seconds|sing_box_service_pid|pid_is_sing_box|sing_box_service_is_running|sing_box_service_is_stable|padkap_evolution_runtime_network_is_configured|padkap_evolution_is_running|padkap_evolution_is_stably_running|wait_for_padkap_evolution_stable_start'
if grep -R -n -E "$service_predicates" "$PADKAP_EVOLUTION_BIN" "$ROOT_DIR/padkap-evolution/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
if grep -R -n -E "$pending_state" "$PADKAP_EVOLUTION_BIN" "$PADKAP_EVOLUTION_INIT" "$ROOT_DIR/padkap-evolution/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
  fail "pending reload/time-sync ownership must not remain in shell"
fi

runtime_state_shell_symbols='runtime_state_ucode|acquire_runtime_dir_lock|release_runtime_dir_lock|capture_reload_state|write_reload_state_file|write_reload_state|clear_reload_state|populate_nft_runtime_sets|rebuild_nft_runtime|reload_sing_box_runtime|apply_pending_urltest_selector_switches|close_inherited_service_lock_fd|PADKAP_EVOLUTION_URLTEST_SELECTOR_SWITCHES'
if grep -R -n -E "$runtime_state_shell_symbols" "$PADKAP_EVOLUTION_BIN" "$PADKAP_EVOLUTION_INIT" "$ROOT_DIR/padkap-evolution/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
  fail "runtime_state.sh symbols must not remain in shell"
fi

if grep -Fq 'acquire_reload_lock' "$PADKAP_EVOLUTION_INIT" || grep -Fq 'release_reload_lock' "$PADKAP_EVOLUTION_INIT"; then
...
if grep -R -n -F 'updates_runtime.sh' "$PADKAP_EVOLUTION_BIN" "$ROOT_DIR/padkap-evolution/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
grep -Fq 'mode == "padkap-evolution-running"' "$STATE_UC" ||
  fail "service/state.uc must own Padkap Evolution running predicate"
grep -Fq 'mode == "sing-box-service-stable"' "$STATE_UC" ||
  fail "service/state.uc must expose stable sing-box predicate"
grep -Fq 'mode == "wait-padkap-evolution-stable-start"' "$STATE_UC" ||
...
grep -Fq '"wait-padkap-evolution-stable-start"' "$LIFECYCLE_UC" ||
...
early_stable_line="$(awk -v start="$sing_box_start_line" 'NR > start && /"wait-padkap-evolution-stable-start"/ { print NR; exit }' "$LIFECYCLE_UC")"
...
grep -Fq 'Reload verification failed after sing-box was reloaded; stopping Padkap Evolution runtime' "$LIFECYCLE_UC" ||
  fail "service/lifecycle.uc must fail reload when sing-box does not stay stable"
grep -Fq 'Reload runtime restart verification failed after Padkap Evolution was started; rolling back DNS changes' "$LIFECYCLE_UC" ||
...
if grep -Fq 'nft-create-full-runtime-from-uci' "$LIFECYCLE_UC"; then
  fail "padkap-evolution start/reload must rebuild nft runtime instead of appending to stale state"
fi
