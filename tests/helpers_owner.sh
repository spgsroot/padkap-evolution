...
HELPERS_SH="$PADKAP_EVOLUTION_LIB/helpers.sh"
LIFECYCLE_UC="$PADKAP_EVOLUTION_LIB/service/lifecycle.uc"
PACKAGES_UC="$PADKAP_EVOLUTION_LIB/core/packages.uc"
RULES_UC="$PADKAP_EVOLUTION_LIB/providers/rules.uc"
SINGBOX_RUNTIME_UC="$PADKAP_EVOLUTION_LIB/singbox/runtime.uc"
COMPONENT_ACTION_UC="$PADKAP_EVOLUTION_LIB/components/action.uc"
DIAGNOSTICS_RUNTIME_UC="$PADKAP_EVOLUTION_LIB/diagnostics/runtime.uc"
WORK_DIR="$(mktemp -d)"
...
if grep -R -n -F 'helpers.sh' "$PADKAP_EVOLUTION_BIN" "$PADKAP_EVOLUTION_LIB" --include='*.sh' >/dev/null 2>&1; then
...
for shell_owner_pattern in \
  ...
  'uci_set'
do
  if grep -Fq "$shell_owner_pattern" "$PADKAP_EVOLUTION_BIN"; then
    fail "padkap-evolution shell entrypoint must not own $shell_owner_pattern logic"
  fi
done
if grep -E -n '(^|[;&|[:space:]])nft[[:space:]]+(list|add|delete|flush)' "$PADKAP_EVOLUTION_BIN" >/dev/null 2>&1; then
  fail "padkap-evolution shell entrypoint must not own nft command logic"
fi
if grep -E -n '(^|[;&|[:space:]])ip[[:space:]]+(rule|route)' "$PADKAP_EVOLUTION_BIN" >/dev/null 2>&1; then
  fail "padkap-evolution shell entrypoint must not own ip rule/route logic"
fi
[ ! -e "$PADKAP_EVOLUTION_LIB/updater.sh" ] ||
...
if ucode -L "$PADKAP_EVOLUTION_LIB" "$PACKAGES_UC" installed padkap-evolution-definitely-missing >/dev/null 2>&1; then
  fail "missing package must not be reported installed"
fi

mark_hex="$(ucode -L "$PADKAP_EVOLUTION_LIB" "$RULES_UC" mark-hex 0x01000000 2)"
[ "$mark_hex" = "0x01000002" ] ||
  fail "providers/rules.uc mark math changed"

version="$(printf 'sing-box version 1.12.4-extended\nEnvironment: test\n' |
  ucode -L "$PADKAP_EVOLUTION_LIB" "$SINGBOX_RUNTIME_UC" version-from-output)"
[ "$version" = "1.12.4-extended" ] ||
  fail "singbox/runtime.uc version-from-output changed"

ucode -L "$PADKAP_EVOLUTION_LIB" "$SINGBOX_RUNTIME_UC" is-extended "1.12.4-extended" >/dev/null ||
  fail "extended sing-box version should be detected"
if ucode -L "$PADKAP_EVOLUTION_LIB" "$SINGBOX_RUNTIME_UC" is-extended "1.12.4" >/dev/null 2>&1; then
  fail "stable sing-box version must not be detected as extended"
fi
ucode -L "$PADKAP_EVOLUTION_LIB" "$SINGBOX_RUNTIME_UC" supports-tailscale "" "$(printf 'Tags: with_quic,with_tailscale\n')" >/dev/null ||
  fail "with_tailscale build tag should be detected"

SB_VERSION_STATE_FILE="$WORK_DIR/version" \
  ucode -L "$PADKAP_EVOLUTION_LIB" "$SINGBOX_RUNTIME_UC" write-version-state 1.2.3 >/dev/null ||
  fail "version state write failed"
[ "$(SB_VERSION_STATE_FILE="$WORK_DIR/version" ucode -L "$PADKAP_EVOLUTION_LIB" "$SINGBOX_RUNTIME_UC" read-version-state)" = "1.2.3" ] ||
  fail "version state read failed"
SB_VERSION_STATE_FILE="$WORK_DIR/version" \
  ucode -L "$PADKAP_EVOLUTION_LIB" "$SINGBOX_RUNTIME_UC" restore-version-state "" >/dev/null ||
...
SB_VARIANT_STATE_FILE="$WORK_DIR/variant" \
  ucode -L "$PADKAP_EVOLUTION_LIB" "$SINGBOX_RUNTIME_UC" write-variant-marker extended-compressed >/dev/null ||
  fail "variant marker write failed"
SB_VARIANT_STATE_FILE="$WORK_DIR/variant" \
  ucode -L "$PADKAP_EVOLUTION_LIB" "$SINGBOX_RUNTIME_UC" marker-is extended-compressed >/dev/null ||
  fail "variant marker check failed"
[ "$(SB_VARIANT_STATE_FILE="$WORK_DIR/variant" ucode -L "$PADKAP_EVOLUTION_LIB" "$SINGBOX_RUNTIME_UC" read-variant-marker)" = "extended-compressed" ] ||
  fail "variant marker read failed"
SB_VARIANT_STATE_FILE="$WORK_DIR/variant" \
  ucode -L "$PADKAP_EVOLUTION_LIB" "$SINGBOX_RUNTIME_UC" restore-variant-marker "" >/dev/null ||
