...
export PADKAP_EVOLUTION_DEADLINE_HELPER_PATH="$deadline_helper"
export PADKAP_EVOLUTION_INSTALLER_DEADLINE_HELPER="$deadline_helper"
export PADKAP_EVOLUTION_INSTALLER_COMMAND_RESULT="$WORK_DIR/installer-command"
...
grep -Fq '/usr/bin/padkap-evolution component_action sing_box "$action"' "$INSTALLER" ||
  fail "selected sing-box install must delegate to padkap-evolution component_action"
...
grep -Fq 'ucode -L /usr/lib/padkap-evolution /usr/lib/padkap-evolution/config/migration.uc migrate-podkop' "$INSTALLER" ||
  fail "install.sh must delegate the legacy transition to the installed migration module"

if grep -n -E 'restore_padkap_evolution_dnsmasq_failsafe|remember_service_state|stop_conflicting_services|deactivate_original_padkap_evolution_if_present|remove_conflicting_dns_proxy|pkg_remove_if_installed|pkg_remove_matching_prefix|pkg_list_installed_names' "$INSTALLER" >/dev/null 2>&1; then
  fail "install.sh must not keep shell cleanup/remove service owners"
fi
if grep -n -E 'rm -f /var/luci-indexcache|rm -f /tmp/luci-indexcache|/etc/init\.d/rpcd[[:space:]]+reload|/etc/init\.d/padkap-evolution[[:space:]]+(start|stop|disable|enable|restart)|/etc/init\.d/padkap-evolution[[:space:]]+(stop|disable)' "$INSTALLER" >/dev/null 2>&1; then
...
case "$1" in
  list-installed)
    ...
    if [ -n "${PADKAP_EVOLUTION_INSTALLER_FAKE_LEGACY_BACKEND:-}" ]; then
      printf '%s\n' \
        "$PADKAP_EVOLUTION_INSTALLER_FAKE_LEGACY_BACKEND - 1.0" \
        "luci-app-$PADKAP_EVOLUTION_INSTALLER_FAKE_LEGACY_BACKEND - 1.0" \
        "luci-i18n-$PADKAP_EVOLUTION_INSTALLER_FAKE_LEGACY_BACKEND-ru - 1.0"
    fi
    ;;
  remove)
    shift
    [ "$1" = "--force-depends" ] && shift
    printf '%s\n' "$1" >> "$PADKAP_EVOLUTION_INSTALLER_OPKG_LOG"
    [ "${PADKAP_EVOLUTION_INSTALLER_FAIL_REMOVE:-}" != "$1" ] || exit 1
    ;;
esac
exit 0
SH
...
#!/usr/bin/env sh
printf '%s\n' "$1" >> "$PADKAP_EVOLUTION_INSTALLER_INIT_LOG"
case "$1" in
  enabled|status|running)
    printf '%s\n' "$$" >> "$PADKAP_EVOLUTION_INSTALLER_HANG_PID_LOG"
    trap '' TERM
    sh -c '
      trap "" TERM
      printf "%s\n" "$$" >> "$PADKAP_EVOLUTION_INSTALLER_HANG_PID_LOG"
...
ORPHAN_PROBE_PID="$(
  PADKAP_EVOLUTION_INSTALLER_INIT_LOG="$WORK_DIR/hanging-init.log" \
  PADKAP_EVOLUTION_INSTALLER_HANG_PID_LOG="$HANG_PID_LOG" \
...
PATH="$WORK_DIR:$PATH" \
PADKAP_EVOLUTION_INSTALLER_OPKG_LOG="$WORK_DIR/opkg.log" \
PADKAP_EVOLUTION_INSTALLER_INIT="$WORK_DIR/hanging-init" \
PADKAP_EVOLUTION_INSTALLER_BIN="$WORK_DIR/hanging-bin" \
PADKAP_EVOLUTION_INSTALLER_LIB="$WORK_DIR/missing-padkap-evolution-lib" \
PADKAP_EVOLUTION_INSTALLER_UCI_DEFAULTS="$WORK_DIR/missing-uci-defaults" \
PADKAP_EVOLUTION_INSTALLER_LUCI_VIEW="$WORK_DIR/missing-luci-view" \
PADKAP_EVOLUTION_INSTALLER_MENU_JSON="$WORK_DIR/missing-menu.json" \
PADKAP_EVOLUTION_INSTALLER_ACL_JSON="$WORK_DIR/missing-acl.json" \
PADKAP_EVOLUTION_INSTALLER_RU_LMO="$WORK_DIR/missing-ru.lmo" \
PADKAP_EVOLUTION_INSTALLER_EN_LMO="$WORK_DIR/missing-en.lmo" \
PADKAP_EVOLUTION_INSTALLER_RU_LUA="$WORK_DIR/missing-ru.lua" \
PADKAP_EVOLUTION_INSTALLER_EN_LUA="$WORK_DIR/missing-en.lua" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_INIT="$WORK_DIR/missing-original-init" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
PADKAP_EVOLUTION_INSTALLER_RC_DIR="$WORK_DIR/rc.d" \
PADKAP_EVOLUTION_INSTALLER_START_RETRY_FILE="$WORK_DIR/start.retry" \
PADKAP_EVOLUTION_INSTALLER_START_RETRY_PID_FILE="$WORK_DIR/start-retry.pid" \
PADKAP_EVOLUTION_INSTALLER_ORPHAN_PPID="$ORPHAN_PROBE_PPID" \
PADKAP_EVOLUTION_INSTALLER_SERVICE_PROBE_TIMEOUT=1 \
PADKAP_EVOLUTION_INSTALLER_SERVICE_ACTION_TIMEOUT=3 \
PADKAP_EVOLUTION_INSTALLER_INIT_LOG="$WORK_DIR/hanging-init.log" \
PADKAP_EVOLUTION_INSTALLER_HANG_PID_LOG="$HANG_PID_LOG" \
PADKAP_EVOLUTION_UCI_STATE_FILE="$WORK_DIR/empty-uci.state" \
...
grep -Fxq 'PADKAP_EVOLUTION_WAS_ENABLED=1' "$WORK_DIR/hanging-state.env" ||
...
printf '%s\n' '1' |
  PATH="$WORK_DIR:$PATH" \
  PADKAP_EVOLUTION_INSTALLER_OPKG_LOG="$WORK_DIR/opkg.log" \
  PADKAP_EVOLUTION_INSTALLER_INIT="$WORK_DIR/missing-padkap-evolution-init" \
  PADKAP_EVOLUTION_INSTALLER_BIN="$WORK_DIR/missing-padkap-evolution-bin" \
  PADKAP_EVOLUTION_INSTALLER_LIB="$WORK_DIR/missing-padkap-evolution-lib" \
  PADKAP_EVOLUTION_INSTALLER_UCI_DEFAULTS="$WORK_DIR/missing-uci-defaults" \
  PADKAP_EVOLUTION_INSTALLER_LUCI_VIEW="$WORK_DIR/missing-luci-view" \
  PADKAP_EVOLUTION_INSTALLER_MENU_JSON="$WORK_DIR/missing-menu.json" \
...
case "$1" in
  enabled) exit 0 ;;
  status) printf '%s\n' running; exit 0 ;;
  stop|disable|enable|start|restart) printf '%s\n' "$1" >> "$PADKAP_EVOLUTION_INSTALLER_INIT_LOG"; exit 0 ;;
esac
exit 0
SH
...
#!/usr/bin/env sh
printf '%s\n' "$*" >> "$PADKAP_EVOLUTION_INSTALLER_BIN_LOG"
case "$1" in
  get_status) printf '%s\n' '{"running":1}' ;;
  restore_dnsmasq) exit "${PADKAP_EVOLUTION_INSTALLER_FAIL_BACKEND_RESTORE:-0}" ;;
esac
exit 0
SH
...
PADKAP_EVOLUTION_INSTALLER_BIN="$WORK_DIR/missing-padkap-evolution-bin" \
PADKAP_EVOLUTION_INSTALLER_LIB="$WORK_DIR/missing-padkap-evolution-lib" \
PADKAP_EVOLUTION_INSTALLER_UCI_DEFAULTS="$WORK_DIR/uci-defaults" \
PADKAP_EVOLUTION_INSTALLER_LUCI_VIEW="$WORK_DIR/luci-view" \
PADKAP_EVOLUTION_INSTALLER_MENU_JSON="$WORK_DIR/menu.json" \
PADKAP_EVOLUTION_INSTALLER_ACL_JSON="$WORK_DIR/acl.json" \
PADKAP_EVOLUTION_INSTALLER_RU_LMO="$WORK_DIR/ru.lmo" \
PADKAP_EVOLUTION_INSTALLER_EN_LMO="$WORK_DIR/en.lmo" \
PADKAP_EVOLUTION_INSTALLER_RU_LUA="$WORK_DIR/ru.lua" \
PADKAP_EVOLUTION_INSTALLER_EN_LUA="$WORK_DIR/en.lua" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_INIT="$WORK_DIR/missing-original-init" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_CONFIG_ALT="$LEGACY_CONFIG_ALT" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_INIT="$WORK_DIR/legacy-init" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BIN="$WORK_DIR/legacy-bin" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_LIB="$WORK_DIR/legacy-lib" \
PADKAP_EVOLUTION_INSTALLER_FAKE_LEGACY_BACKEND="$LEGACY_BACKEND" \
PADKAP_EVOLUTION_INSTALLER_OPKG_LOG="$WORK_DIR/opkg.log" \
PADKAP_EVOLUTION_INSTALLER_INIT_LOG="$WORK_DIR/init.log" \
PADKAP_EVOLUTION_INSTALLER_BIN_LOG="$WORK_DIR/bin.log" \
PADKAP_EVOLUTION_INSTALLER_FAIL_BACKEND_RESTORE=1 \
PADKAP_EVOLUTION_UCI_STATE_FILE="$WORK_DIR/empty-uci.state" \
  ucode "$helper" installer-cleanup-legacy >"$state"

grep -Fxq 'PADKAP_EVOLUTION_WAS_ENABLED=1' "$state" ||
  fail "installer cleanup must export previous enabled state"
grep -Fxq 'PADKAP_EVOLUTION_WAS_RUNNING=1' "$state" ||
  fail "installer cleanup must export previous running state"
grep -Fxq 'PADKAP_EVOLUTION_LEGACY_DETECTED=1' "$state" ||
...
if printf '%s\n' '1' |
  PATH="$WORK_DIR:$PATH" \
  PADKAP_EVOLUTION_INSTALLER_OPKG_LOG="$WORK_DIR/opkg.log" \
  PADKAP_EVOLUTION_INSTALLER_FAIL_REMOVE="$LEGACY_BACKEND" \
  PADKAP_EVOLUTION_INSTALLER_FAKE_LEGACY_BACKEND="$LEGACY_BACKEND" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
  PADKAP_EVOLUTION_UCI_STATE_FILE="$WORK_DIR/empty-uci.state" \
...
padkap_evolution_persistent="$WORK_DIR/padkap-evolution-persistent"
...
mkdir -p \
  ...
  "$padkap_evolution_persistent/tailscale/server-existing" \
...
printf '%s\n' 'current-padkap-evolution-identity' >"$padkap_evolution_persistent/tailscale/server-existing/node.key"
...
PADKAP_EVOLUTION_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_CONFIG_ALT="$LEGACY_CONFIG_ALT" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_CONFIG="$legacy_config" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_CONFIG_FILE_ALT="$legacy_config_alt" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_PERSISTENT_DIR="$legacy_persistent" \
PADKAP_EVOLUTION_INSTALLER_PERSISTENT_DIR="$padkap_evolution_persistent" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_RUNTIME_DIR="$legacy_runtime" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_TMP_DIR="$legacy_tmp" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_TMP_ALT_DIR="$legacy_tmp_alt" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_CONFIG="$legacy_base_config" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_PERSISTENT_DIR="$legacy_base_persistent" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_RUNTIME_DIR="$legacy_base_runtime" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_TMP_DIR="$legacy_base_tmp" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_INIT="$legacy_base_init" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_BIN="$legacy_base_bin" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_LIB="$legacy_base_lib" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_UCI_DEFAULTS="$legacy_base_uci_defaults" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_LUCI_VIEW="$legacy_base_luci_view" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_MENU_JSON="$legacy_base_menu" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_ACL_JSON="$legacy_base_acl" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_I18N="$legacy_base_i18n" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_TMP_PACKAGE_GLOB="$WORK_DIR/*${LEGACY_BRAND}*" \
PADKAP_EVOLUTION_INSTALLER_LEGACY_SCAN_ROOTS="$legacy_scan_root" \
  ucode "$helper" installer-finalize-legacy
[ "$(cat "$padkap_evolution_persistent/tailscale/server-new/node.key")" = 'legacy-node-identity' ] ||
  fail "installer finalization must migrate legacy Tailscale node identity"
[ "$(cat "$padkap_evolution_persistent/tailscale/server-existing/node.key")" = 'current-padkap-evolution-identity' ] ||
  fail "installer finalization must not overwrite existing Padkap Evolution Tailscale state"
...
padkap_evolution_persistent_file="$WORK_DIR/padkap-evolution-persistent-file"
touch "$padkap_evolution_persistent_file"
if PADKAP_EVOLUTION_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_CONFIG_ALT="$LEGACY_CONFIG_ALT" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_CONFIG="$legacy_config" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_CONFIG_FILE_ALT="$legacy_config_alt" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_PERSISTENT_DIR="$legacy_persistent" \
  PADKAP_EVOLUTION_INSTALLER_PERSISTENT_DIR="$padkap_evolution_persistent_file" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_RUNTIME_DIR="$legacy_runtime" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_TMP_DIR="$legacy_tmp" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_TMP_ALT_DIR="$legacy_tmp_alt" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_CONFIG="$legacy_base_config" \
  ...
  PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_ACL_JSON="$legacy_base_acl" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_BASE_I18N="$legacy_base_i18n" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_TMP_PACKAGE_GLOB="$WORK_DIR/missing-${LEGACY_BRAND}*" \
  PADKAP_EVOLUTION_INSTALLER_LEGACY_SCAN_ROOTS="$legacy_scan_root" \
...
write_fake_service_init "$WORK_DIR/padkap-evolution-init"
touch "$WORK_DIR/luci-indexcache.one" "$WORK_DIR/luci-indexcache.two"
: >"$WORK_DIR/init.log"
PADKAP_EVOLUTION_INSTALLER_INIT="$WORK_DIR/padkap-evolution-init" \
PADKAP_EVOLUTION_INSTALLER_RPCD_INIT="$WORK_DIR/missing-rpcd" \
PADKAP_EVOLUTION_INSTALLER_LUCI_CACHE_GLOBS="$WORK_DIR/luci-indexcache*" \
PADKAP_EVOLUTION_INSTALLER_LATEST_VERSION_CACHE="$WORK_DIR/latest.cache" \
PADKAP_EVOLUTION_INSTALLER_SYSTEM_INFO_CACHE="$WORK_DIR/system-info.json" \
PADKAP_EVOLUTION_INSTALLER_SERVER_COUNTRY_CACHE="$WORK_DIR/server-country.json" \
PADKAP_EVOLUTION_INSTALLER_SING_BOX_VERSION_CACHE="$WORK_DIR/sing-box-version" \
PADKAP_EVOLUTION_INSTALLER_TMP_SYSTEM_INFO_CACHE="$WORK_DIR/tmp-system-info.json" \
PADKAP_EVOLUTION_WAS_ENABLED=1 \
PADKAP_EVOLUTION_WAS_RUNNING=1 \
PADKAP_EVOLUTION_INSTALLER_INIT_LOG="$WORK_DIR/init.log" \
